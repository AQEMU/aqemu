...
First_Start_Wizard::First_Start_Wizard(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);
...
void First_Start_Wizard::on_Button_Cancel_clicked()
{
  if (QMessageBox::information(
          this, tr("Warning!"),
          tr("Are you sure? You can set these options at any time in the "
             "configuration dialog. (File -> Configuration)"),
...
void First_Start_Wizard::on_Button_Next_clicked()
{
  ...
  if (ui.All_Pages->currentIndex() ==
      3)  //FIXME: skipping this page but it could maybe be removed
    ui.All_Pages->setCurrentIndex(ui.All_Pages->currentIndex() + 1);

  if (ui.All_Pages->currentWidget() == ui.Find_Emulators_Page) {
    if (!Emulators_Find_Done)
...
void First_Start_Wizard::on_Button_Find_Emulators_clicked()
{
  ...
  if (sys_env.count() <= 0) {
    ...
    for (int ix = 0; ix < paths.count(); ix++) {
      if (paths[ix].contains("/usr/bin/X11"))
        paths.removeAt(ix);
    }
    ...
    // Add / to line end
    for (int ix = 0; ix < paths.count(); ++ix) {
      paths[ix] = QDir::toNativeSeparators(
    ...
    if (paths.count() <= 0) {
      ...
      if (qemuIMG_Found)
        AQDebug("void First_Start_Wizard::on_Button_Find_Emulators_clicked()",
                "qemu-img find on: " +
                    Settings.value("QEMU-IMG_Path", "").toString());
      else
      ...
      for (int qx = 0; qx < paths.count(); ++qx) {
        ...
        for (QMap<QString, QString>::const_iterator it = qemu_list.constBegin();
             it != qemu_list.constEnd(); ++it) {
          if (!it.value().isEmpty()) {
        ...
        QProgressDialog progressWin(tr("Search..."), tr("Cancel"), 0,
                                    allEmulBinCount, this);
        progressWin.setWindowModality(Qt::WindowModal);

        QMap<QString, Available_Devices> devList;

        iter = qemu_list.constBegin();
        for (int emulBinIndex = 0; iter != qemu_list.constEnd();
             ++emulBinIndex) {
          ...
          if (!iter.value().isEmpty()) {
            ...
            Available_Devices tmpDev = System_Info::Get_Emulator_Info(
                iter.value(), &ok, qemu_version, iter.key());

            if (ok)
              devList[iter.key()] = tmpDev;
            else
              AQGraphic_Warning(
                  "void First_Start_Wizard::on_Button_Find_clicked()",
                  tr("Error!"),
                  tr("Cannot get emulator info! For file: %1")
                      .arg(iter.value()));
          }
          ++iter;
        ...
        // Add Text
        ui.Edit_Enulators_List->appendPlainText(
            tr("QEMU Found in \"%1\", version: %2")
                .arg(paths[qx])
                .arg(Emulator_Version_To_String(qemu_version)));
      }
...
void First_Start_Wizard::on_Button_Edit_clicked()
{
  ...
  if (edit_win->exec() == QDialog::Accepted) {
    ...
    ui.Edit_Enulators_List->setPlainText(
        tr("Emulator Version Modified By User"));
  }
...
{
  QString emulatorDirPath = QFileDialog::getExistingDirectory(
      this, tr("Select QEMU emulator directory"),
      Get_Last_Dir_Path(ui.Edit_Add_Emulator_Path->text()));

  if (!emulatorDirPath.isEmpty())
    ui.Edit_Add_Emulator_Path->setText(
        QDir::toNativeSeparators(emulatorDirPath));
}
...
#ifdef Q_OS_WIN32
  ui.Edit_Add_Emulator_Path->setText(
      QDir::toNativeSeparators(QDir::currentPath() + "/QEMU/"));
  ui.Edit_VM_Dir->setText(QDir::toNativeSeparators(
      Settings.value("VM_Directory", QDir::homePath() + "/AQEMU_VM/")
          .toString()));
#else
  ui.Edit_VM_Dir->setText(QDir::toNativeSeparators(
...
bool First_Start_Wizard::Save_Settings()
{
  ...
  if (dir.exists(ui.Edit_VM_Dir->text())) {
    if (!dir.exists(ui.Edit_VM_Dir->text() +
      ...
      dir.mkdir(ui.Edit_VM_Dir->text() +
                QDir::toNativeSeparators("/os_templates/"));
  }
  else if (!(dir.mkdir(ui.Edit_VM_Dir->text()) &&
             dir.mkdir(ui.Edit_VM_Dir->text() +
                       QDir::toNativeSeparators("/os_templates/")))) {
  ...
  if (ui.CB_Language->currentIndex() == 0)
    ...
    Settings.setValue("Language",
                      ui.CB_Language->itemText(ui.CB_Language->currentIndex()));
  ...
  if (Emul.Get_Name().isEmpty()) {
    ...
    while (true) {
      ...
      if (!foundEmulBinary) {
        ...
        searchAgain = false;
        Emul.Set_Binary_Files(System_Info::Find_QEMU_Binary_Files(
    ...
    if (!foundEmulBinary) {
      AQGraphic_Warning(tr("Error"),
                        tr("Emulator binary files not exists! Please click "
                           "Back and set it manual."));
      return false;
    }
    ...
    if (!QFile::exists(Settings.value("QEMU-IMG_Path", "").toString())) {
      QString qemuImgPath =
          (ui.Edit_Add_Emulator_Path->text().endsWith("/") ||
           ui.Edit_Add_Emulator_Path->text().endsWith("\\"))
              ? ui.Edit_Add_Emulator_Path->text() + "qemu-img.exe"
              : ui.Edit_Add_Emulator_Path->text() +
                    QDir::toNativeSeparators("/qemu-img.exe");

      if (QFile::exists(qemuImgPath))
        Settings.setValue("QEMU-IMG_Path",
                          QDir::toNativeSeparators(qemuImgPath));
      else
        AQGraphic_Warning(
            tr("Warning!"),
            tr("Cannot find qemu-img.exe! You most set it path in: "
               "File->Advanced Settings->Advanced->qemu-img path"));
    }
  }
...
void First_Start_Wizard::retranslateUi()
{
  ...
  if (ui.CB_Language->currentIndex() != 0) {
    if (appTranslator.load(
      ...
      AQError("void First_Start_Wizard::retranslateUi()",
  ...
  ui.Label_Welcome_Text->setText(
      tr("Welcome to the AQEMU settings wizard!\nThis wizard will help you to "
         "choose options AQEMU needs to work correctly. Click on \"Next\" to "
         "go to next page or the \"Back\" button to go to the previous page."));
  ui.Label_Select_Language->setText(
      tr("Here you can choose the interface language"));
  ui.Label_VM_Dir->setText(
      tr("Please set the folder for virtual machine configurations:"));
  ui.Label_Find_Emulators->setText(tr(
      "To work correctly, AQEMU must find QEMU. To search automatically, click "
      "on \"Search\". If the search can't find QEMU, you can reconfigure AQEMU "
      "later. You can do it in the \"File->Settings\" dialog."));
  ...
  ui.Label_Finish_Text->setText(
      tr("Congratulations!\n\nYou can now configure and use virtual "
