...
void Convert_HDD_Thread::run()
{
  ...
  QSettings settings;
  proc->start(settings.value("QEMU-IMG_Path", "qemu-img").toString(),
              Arguments);
  ...
  if (err_str.size() > 0) {
    AQError("void Convert_HDD_Thread::run()",
            "qemu-img Send Error String!\nDetalis: " + err_str);
    ...
    : QDialog(parent)
{
  ...
  connect(HDD_Info, SIGNAL(Completed(bool)), this, SLOT(Update_Info(bool)));
  Valid_Info = false;
...
void Convert_HDD_Image_Window::on_Button_Browse_Base_clicked()
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Select Base HDD Image File"),
...
void Convert_HDD_Image_Window::on_Button_Browse_Output_clicked()
{
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Out HDD Image File"),
...
void Convert_HDD_Image_Window::on_CB_Output_Format_currentIndexChanged(
    const QString& text)
{
...
void Convert_HDD_Image_Window::on_Button_Convert_clicked()
{
  if (ui.Edit_Output_File_Name->text().isEmpty()) {
    AQGraphic_Warning(tr("File Name is Empty!"),
                      tr("Please Enter Correct HDD Image filename!"));
    return;
  }

  if (!Valid_Info) {
    AQGraphic_Warning(tr("Error!"),
                      tr("Cannot get a valid format for current HDD image!"));
    return;
  }
  ...
  // compressed
  if (ui.CH_Compressed->isEnabled() && ui.CH_Compressed->isChecked())
    args << "-c";
  ...
  connect(&Conv_Thread, SIGNAL(Conversion_Complete(bool)), this,
          SLOT(Conversion_Done(bool)));

  Conv_Thread.Set_Args(args);
  Conv_Thread.start();

  ProgDial = new QProgressDialog(tr("Please Wait. Converting HDD Image..."),
                                 tr("Cancel"), 0, 0, this);

  connect(ProgDial, SIGNAL(canceled()), this, SLOT(Cancel_Convertion()));
...
void Convert_HDD_Image_Window::Conversion_Done(bool ok)
{
  if (ok) {
    ...
    AQGraphic_Warning(tr("Error!"), tr("Cannot convert image!\nDetails:\n%1")
                                        .arg(Conv_Thread.Get_Error_Message()));
  }
}
...
bool Convert_HDD_Image_Window::Get_QEMU_IMG_Info()
{
  ...
  qemuImgProc->start(settings.value("QEMU-IMG_Path", "qemu-img").toString(),
                     QStringList("-h"));
  ...
  else  // Parse text
  {
    ...
    if (qcow2 != -1)
      ui.CB_Output_Format->setCurrentIndex(qcow2);
    else {
      // qcow2 not available use qcow
      int qcow = ui.CB_Output_Format->findText("qcow");
      if (qcow != -1)
        ui.CB_Output_Format->setCurrentIndex(qcow);
    }
