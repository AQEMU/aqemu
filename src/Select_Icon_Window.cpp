...
Select_Icon_Window::Select_Icon_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);

  // load all icons
  QDir* icons_dir = new QDir(QDir::toNativeSeparators(
      Settings.value("AQEMU_Data_Folder", "").toString() + "/os_icons/"));
  QFileInfoList icon_files =
      icons_dir->entryInfoList(QDir::Files, QDir::Name | QDir::IgnoreCase);

  for (int ix = 0; ix < icon_files.count(); ++ix) {
    QListWidgetItem* ic =
        new QListWidgetItem(QIcon(icon_files[ix].absoluteFilePath()),
...
void Select_Icon_Window::Set_Previous_Icon_Path(const QString& path)
{
  ...
  {
    if (path.contains("linux", Qt::CaseInsensitive))
      ui.RB_Icon_Linux->setChecked(true);
    else if (path.contains("windows", Qt::CaseInsensitive))
      ui.RB_Icon_Windows->setChecked(true);
    else
      ui.RB_Icon_Other->setChecked(true);
  }
  else if (path.indexOf(QDir::toNativeSeparators(
               Settings.value("AQEMU_Data_Folder", "").toString() +
               "/os_icons/")) == 0)  // AQEMU Icons Folder
  {
...
void Select_Icon_Window::on_Button_Browse_clicked()
{
  ...
  if (!QFile::exists(iconPath)) {
    AQError("void Select_Icon_Window::done(int)", "File No Exists!");
  }
  else {
...
void Select_Icon_Window::on_All_Icons_List_itemDoubleClicked(
