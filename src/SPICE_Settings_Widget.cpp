...
SPICE_Settings_Widget::SPICE_Settings_Widget(QWidget* parent) : QWidget(parent)
{
  ui.setupUi(this);
  ...
  connect(ui.Edit_SPICE_Port, SIGNAL(valueChanged(int)), this,
          SIGNAL(State_Changed()));
  connect(ui.CH_SPICE_SPort, SIGNAL(clicked()), this, SIGNAL(State_Changed()));
  connect(ui.Edit_SPICE_SPort, SIGNAL(valueChanged(int)), this,
          SIGNAL(State_Changed()));
  connect(ui.CH_SPICE_Host, SIGNAL(clicked()), this, SIGNAL(State_Changed()));
  connect(ui.Edit_SPICE_Host, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(State_Changed()));

  connect(ui.CH_Use_Image_Compression, SIGNAL(clicked()), this,
          SIGNAL(State_Changed()));
  connect(ui.CB_Image_Compression, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(State_Changed()));
  ...
  connect(ui.CH_Use_Video_Compression, SIGNAL(clicked()), this,
          SIGNAL(State_Changed()));
  connect(ui.CH_Use_Playback_Compression, SIGNAL(clicked()), this,
          SIGNAL(State_Changed()));

  connect(ui.RB_No_Password, SIGNAL(toggled(bool)), this,
          SIGNAL(State_Changed()));
  connect(ui.Edit_Password, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(State_Changed()));
...
const VM_SPICE& SPICE_Settings_Widget::Get_Settings(bool& settingsValidated)
{
  ...
  if (ui.CB_Image_Compression->currentIndex() == 0)
    ...
    AQGraphic_Warning(tr("Error"), tr("SPICE image compression type invalid!"));
    settingsValidated = false;
    return spiceSettings;
  }
  ...
  for (int ix = 0; ix < ui.Renderer_Order_List->count(); ++ix) {
    tmpRendersList << (VM::SPICE_Renderer)ui.Renderer_Order_List->item(ix)
                          ->data(Qt::UserRole)
                          .toInt();
  }
  spiceSettings.Set_Renderer_List(tmpRendersList);

  spiceSettings.Use_Video_Stream_Compression(
      ui.CH_Use_Video_Compression->isChecked());
  spiceSettings.Use_Playback_Compression(
      ui.CH_Use_Playback_Compression->isChecked());
...
void SPICE_Settings_Widget::Set_Settings(const VM_SPICE& settings)
{
  ...
  ui.CH_Use_Video_Compression->setChecked(
      settings.Use_Video_Stream_Compression());
  ui.CH_Use_Playback_Compression->setChecked(
      settings.Use_Playback_Compression());
...
void SPICE_Settings_Widget::on_TB_Up_clicked()
{
  if (!ui.Renderer_Order_List->currentItem())
    return;
  if (ui.Renderer_Order_List->row(ui.Renderer_Order_List->currentItem()) <= 0)
    return;

  // Swap
  QString tmpStr = ui.Renderer_Order_List
                       ->item(ui.Renderer_Order_List->row(
                           ui.Renderer_Order_List->currentItem()))
                       ->text();
  VM::SPICE_Renderer tmpRender = (VM::SPICE_Renderer)ui.Renderer_Order_List
                                     ->item(ui.Renderer_Order_List->row(
                                         ui.Renderer_Order_List->currentItem()))
                                     ->data(Qt::UserRole)
                                     .toInt();

  ui.Renderer_Order_List
      ->item(ui.Renderer_Order_List->row(ui.Renderer_Order_List->currentItem()))
      ->setText(ui.Renderer_Order_List
                    ->item(ui.Renderer_Order_List->row(
                               ui.Renderer_Order_List->currentItem()) -
                           1)
                    ->text());
  ...
  ui.Renderer_Order_List
      ->item(
          ui.Renderer_Order_List->row(ui.Renderer_Order_List->currentItem()) -
          1)
      ->setText(tmpStr);
  ui.Renderer_Order_List
      ->item(
          ui.Renderer_Order_List->row(ui.Renderer_Order_List->currentItem()) -
          1)
      ->setData(Qt::UserRole, tmpRender);

  // Select item
  ui.Renderer_Order_List->setCurrentRow(
      ui.Renderer_Order_List->row(ui.Renderer_Order_List->currentItem()) - 1);
}
...
void SPICE_Settings_Widget::on_TB_Down_clicked()
{
  if (!ui.Renderer_Order_List->currentItem())
    return;
  if (ui.Renderer_Order_List->row(ui.Renderer_Order_List->currentItem()) >=
      ui.Renderer_Order_List->count() - 1)
    return;
...
void SPICE_Settings_Widget::Show_Renderer_List(
    const QList<VM::SPICE_Renderer>& list)
{
  ...
  for (int ix = 0; ix < list.count(); ++ix) {
    ...
    switch (list[ix]) {
      ...
      default:
        renderStr = tr("Error!");
        AQError(
            "void SPICE_Settings_Widget::Show_Renderer_List( const "
            "QList<VM::SPICE_Renderer> &list )",
            "Invalid SPICE render type!");
        break;
    }

    QListWidgetItem* tmpItem = new QListWidgetItem(
        renderStr, ui.Renderer_Order_List, QListWidgetItem::Type);
