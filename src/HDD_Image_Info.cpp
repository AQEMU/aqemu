...
HDD_Image_Info::HDD_Image_Info(QObject* parent) : QObject(parent)
{
...
void HDD_Image_Info::Update_Disk_Info(const QString& path)
{
  ...
  if (QFile::exists(Info.Image_File_Name) == false) {
    ...
    QEMU_IMG_Proc = new QProcess(this);
    QSettings settings;
    QEMU_IMG_Proc->start(settings.value("QEMU-IMG_Path", "qemu-img").toString(),
                         args);

    connect(QEMU_IMG_Proc, SIGNAL(finished(int, QProcess::ExitStatus)), this,
            SLOT(Parse_Info(int, QProcess::ExitStatus)), Qt::DirectConnection);

    connect(QEMU_IMG_Proc, SIGNAL(error(QProcess::ProcessError)), this,
            SLOT(Clear_Info()), Qt::DirectConnection);
  }
}

void HDD_Image_Info::Clear_Info()
{
  AQDebug("void HDD_Image_Info::Clear_Info()", "HDD Info Not Read!");
...
void HDD_Image_Info::Parse_Info(int exitCode, QProcess::ExitStatus exitStatus)
{
  ...
  if (info_str.isEmpty()) {
    AQDebug(
        "void HDD_Image_Info::Parse_Info( int exitCode, QProcess::ExitStatus "
        "exitStatus )",
        "Data is empty.");
    return;
  }
  ...
  auto RegInfo = QRegularExpression(
      ".*image:[\\s]+([^\n\r]+).*file format:[\\s]+([\\w\\d]+).*virtual "
      "size:[\\s]+([\\d]+[.]*[\\d]*[KMG]+).*disk "
      "size:[\\s]+([\\d]+[.]*[\\d]*[KMG]+).*cluster_size:[\\s]+([\\d]+).*");

  bool cluster = true;
  if (!RegInfo.match(info_str).hasMatch()) {
    AQWarning(
        "void QEMU_IMG_Thread::Parse_Info( int exitCode, QProcess::ExitStatus "
        "exitStatus )",
        "QRegExp With Cluster Size Not Matched!");

    RegInfo = QRegularExpression(
        QString(".*image:[\\s]+([^\n\r]+).*") +
    ...
    if (!RegInfo.match(info_str).hasMatch()) {
      AQError(
          "void QEMU_IMG_Thread::Parse_Info( int exitCode, "
          "QProcess::ExitStatus exitStatus )",
          "QRegExp Without Cluster Size Not Matched! Image: " +
  ...
  if (cluster)
    Info.Cluster_Size = info_lines[5].toInt();
  else
    Info.Cluster_Size = 0;
