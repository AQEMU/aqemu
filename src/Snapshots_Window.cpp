...
Snapshots_Window::Snapshots_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);
}
    ...
    for (int ix = 0; ix < Current_VM->Get_Snapshots().count(); ++ix) {
      ...
      if (tmp_str == name) {
        ui.Label_ID->setText(tr("ID:") + " " +
                             Current_VM->Get_Snapshots()[ix].Get_ID());
        ui.Label_Size->setText(tr("Size:") + " " +
                               Current_VM->Get_Snapshots()[ix].Get_Size());
        ui.Label_Date->setText(tr("Date:") + " " +
                               Current_VM->Get_Snapshots()[ix].Get_Date());
        ui.Label_VM_Clock->setText(
            tr("VM Clock:") + " " +
            Current_VM->Get_Snapshots()[ix].Get_VM_Clock());
        ui.Label_Description->setText(
            tr("Description:") + " " +
            Current_VM->Get_Snapshots()[ix].Get_Description());
      }
    }
  }
}
...
{
  if (ui.Snapshots_Tree->currentItem() == NULL)
    return;

  if (ui.Snapshots_Tree->currentItem()->text(0).isEmpty()) {
    AQGraphic_Warning(tr("Warning!"),
                      tr("Please select snapshot that you want to delete!"));
    return;
  }

  int ret = QMessageBox::question(
      this, tr("Warning"), tr("Delete Current Snapshot?"),
...
void Snapshots_Window::on_Button_Start_clicked()
{
  ...
  if (Current_VM->Get_State() == VM::VMS_Power_Off ||
    ...
    if (ui.Snapshots_Tree->currentItem()->text(0).isEmpty()) {
      AQGraphic_Warning(tr("Warning!"),
                        tr("Please select snapshot that you want to start!"));
      return;
    }
    else {
      if (Current_VM->Start_Snapshot(
              ui.Snapshots_Tree->currentItem()->text(0))) {
        if (Settings.value("Show_Emulator_Control_Window", "yes").toString() ==
            "yes") {
    ...
    int ret = QMessageBox::question(
        this, tr("Warning"),
        tr("This VM is Running now! Stop VM and Load Snapshot?"),
        QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
...
void Snapshots_Window::on_Button_Properties_clicked()
{
  ...
  if (sel_list.count() > 0) {
    ...
    for (int ix = 0; ix < Current_VM->Get_Snapshots().count(); ++ix) {
      ...
      if (tmp_str == tag) {
        if (Current_VM->Get_Snapshots()[ix].Get_Name().isEmpty()) {
          prop_win->Set_Snapshot_Name(
              Current_VM->Get_Snapshots()[ix].Get_Tag());
        }
        else {
          prop_win->Set_Snapshot_Name(
              Current_VM->Get_Snapshots()[ix].Get_Name());
        }

        prop_win->Set_Snapshot_Description(
            Current_VM->Get_Snapshots()[ix].Get_Description());
...
bool Snapshots_Window::Update_Info()
{
  ...
  if (QFile::exists(Current_VM->Get_HDA().Get_File_Name()))
    imagePath = Current_VM->Get_HDA().Get_File_Name();
  else if (QFile::exists(Current_VM->Get_HDB().Get_File_Name()))
    imagePath = Current_VM->Get_HDB().Get_File_Name();
  else if (QFile::exists(Current_VM->Get_HDC().Get_File_Name()))
    imagePath = Current_VM->Get_HDC().Get_File_Name();
  else if (QFile::exists(Current_VM->Get_HDD().Get_File_Name()))
    imagePath = Current_VM->Get_HDD().Get_File_Name();
  else if (Current_VM->Get_Storage_Devices_List().isEmpty() == false) {
    for (int ix = 0; ix < Current_VM->Get_Storage_Devices_List().count();
         ++ix) {
      if (Current_VM->Get_Storage_Devices_List()[ix].Use_Media() == false ||
          Current_VM->Get_Storage_Devices_List()[ix].Get_Media() ==
              VM::DM_Disk) {
        if (QFile::exists(
                Current_VM->Get_Storage_Devices_List()[ix].Get_File_Path()))
          imagePath =
              Current_VM->Get_Storage_Devices_List()[ix].Get_File_Path();
      }
    }
  ...
  if (imagePath.isEmpty()) {
    AQGraphic_Warning(tr("Error!"), tr("You must add HDD image in QCOW2 format "
  ...
  qimg->start(Settings.value("QEMU-IMG_Path", "qemu-img").toString(),
              qimg_args);
  ...
  auto RegInfo = QRegularExpression(
      ".*Snapshot "
      "list:.*ID[\\s]+TAG[\\s]+VM[\\s]+SIZE[\\s]+DATE[\\s]+VM[\\s]+CLOCK(.+)");

  if (!RegInfo.match(info_str).hasMatch()) {
    AQError("bool Snapshots_Window::Update_Info()", "QRegExp Not Matched!");
    return false;
  }
  ...
  if (info_lines.count() == 2) {
    ...
    auto snap_regexp = QRegularExpression(
        "([\\d])+[\\s]+([a-zA-Z0-9_]+)[\\s]+([0-9KMG]+|[0-9]+.[0-9KMG]+)[\\s]+("
        ".+)[\\s]{2,}(.+)");

    for (int sx = 0; sx < snapshots_list.count(); ++sx) {
      if (!snap_regexp.match(snapshots_list[sx]).hasMatch())
        continue;

      QStringList snap_info =
          snap_regexp.match(snapshots_list[sx]).capturedTexts();

      if (snap_info.count() != 6)
        continue;
