...
Settings_Widget::Settings_Widget(QTabWidget* tab_widget,
                                 QBoxLayout::Direction dir, bool erase_margins,
                                 bool erase_parent_margins)
{
  ...
  while ((widget = tab_widget->widget(i)) != 0) {
    list->addItem(
        new QListWidgetItem(tab_widget->tabIcon(i), tab_widget->tabText(i)));
...
QMap<QString, QList<Settings_Widget*>> Settings_Widget::groups =
    QMap<QString, QList<Settings_Widget*>>();
