...
Add_New_Device_Window::Add_New_Device_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);
}
...
void Add_New_Device_Window::Set_Device(const VM_Native_Storage_Device& dev)
{
  ...
  switch (Device.Get_Interface()) {
    ...
    default:
      AQError(
          "void Add_New_Device_Window::Set_Device( const "
          "VM_Native_Storage_Device &dev )",
  ...
  if (index != -1)
    ui.CB_AIO->setCurrentIndex(index);
  else
    AQError(
        "void Add_New_Device_Window::Set_Device( const "
        "VM_Native_Storage_Device &dev )",
...
void Add_New_Device_Window::Set_Emulator_Devices(
    const Available_Devices& devices)
{
...
void Add_New_Device_Window::on_CB_Interface_currentIndexChanged(
    const QString& text)
{
...
void Add_New_Device_Window::on_TB_File_Path_Browse_clicked()
{
  QString file_name = QFileDialog::getOpenFileName(
      this, tr("Select your device"),
      Get_Last_Dir_Path(ui.Edit_File_Path->text()), tr("All Files (*)"));
