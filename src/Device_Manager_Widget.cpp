...
Device_Manager_Widget::Device_Manager_Widget(QWidget* parent) : QWidget(parent)
{
  ui.setupUi(this);
  ...
  connect(HDA_Info, SIGNAL(Completed(bool)), this, SLOT(Update_HDA(bool)));

  connect(HDB_Info, SIGNAL(Completed(bool)), this, SLOT(Update_HDB(bool)));

  connect(HDC_Info, SIGNAL(Completed(bool)), this, SLOT(Update_HDC(bool)));

  connect(HDD_Info, SIGNAL(Completed(bool)), this, SLOT(Update_HDD(bool)));
...
void Device_Manager_Widget::Set_VM(const Virtual_Machine& vm)
{
  ...
  if (vm.Get_Current_Emulator_Devices()->PSO_Drive)
...
void Device_Manager_Widget::Update_Enabled_Actions()
{
  ...
  if (HDA.Get_Enabled() && HDB.Get_Enabled() && HDC.Get_Enabled() &&
      HDD.Get_Enabled()) {
    ui.actionAdd_HDD->setEnabled(false);
    ui.TB_Add_HDD->setEnabled(false);
  }
  else if (HDA.Get_Enabled() && HDB.Get_Enabled() && HDD.Get_Enabled() &&
           CD_ROM.Get_Enabled()) {
    ui.actionAdd_HDD->setEnabled(false);
    ui.TB_Add_HDD->setEnabled(false);
  }
  ...
  if (ui.Devices_List->currentItem() != NULL) {
    ...
    if (ui.Devices_List->currentItem()->data(512).toString() == "fd1" ||
      ...
      if (It_Host_Device(CD_ROM.Get_File_Name())) {
        ...
        if (cd_img.exists()) {
          ...
          if (size_in_bytes <= 0) {
            ...
            ui.Label_Connected_To->setText(
                tr("Type: Image") + "\n" + tr("On Disk Size: ") +
                QString::number((float)(size_in_bytes / 1024.0 / 1024.0), 'f',
                                2) +
                tr("MB"));
          }
        }
      ...
      if (ui.Devices_List->currentItem()->data(512).toString() == "hda") {
        ...
        ui.Label_Connected_To->setText(
            tr("Image Virtual Size: ") +
            QString::number(HDA.Get_Virtual_Size().Size) +
            Get_TR_Size_Suffix(HDA.Get_Virtual_Size()) + "\n" +
            tr("Image On Disk Size: ") +
            QString::number(HDA.Get_Disk_Size().Size) +
        ...
        ui.Label_Connected_To->setText(
            tr("Image Virtual Size: ") +
            QString::number(HDB.Get_Virtual_Size().Size) +
            Get_TR_Size_Suffix(HDB.Get_Virtual_Size()) + "\n" +
            tr("Image On Disk Size: ") +
            QString::number(HDB.Get_Disk_Size().Size) +
        ...
        ui.Label_Connected_To->setText(
            tr("Image Virtual Size: ") +
            QString::number(HDC.Get_Virtual_Size().Size) +
            Get_TR_Size_Suffix(HDC.Get_Virtual_Size()) + "\n" +
            tr("Image On Disk Size: ") +
            QString::number(HDC.Get_Disk_Size().Size) +
        ...
        ui.Label_Connected_To->setText(
            tr("Image Virtual Size: ") +
            QString::number(HDD.Get_Virtual_Size().Size) +
            Get_TR_Size_Suffix(HDD.Get_Virtual_Size()) + "\n" +
            tr("Image On Disk Size: ") +
            QString::number(HDD.Get_Disk_Size().Size) +
      ...
      for (int fx = 0; fx < 32; ++fx) {
        if (ui.Devices_List->currentItem()->data(512).toString() ==
            "device" + QString::number(fx)) {
          found = true;
...
void Device_Manager_Widget::on_Devices_List_customContextMenuRequested(
    const QPoint& pos)
{
  ...
  if (it != NULL) {
    if (it->data(512).toString() == "fd1" ||
      ...
      if (!found) {
        AQError(
            "void "
            "Device_Manager_Widget::on_Devices_List_customContextMenuRequested("
            " const QPoint &pos )",
            "Incorrect Device!");
      }
    }
...
void Device_Manager_Widget::on_Devices_List_itemDoubleClicked(
...
void Device_Manager_Widget::on_actionAdd_Floppy_triggered()
{
  if (!Floppy1.Get_Enabled()) {
    ...
    AQGraphic_Warning(tr("Warning!"), tr("Maximum Floppy Disk Count is 2"));
  }
}
  ...
  if (!CD_ROM.Get_Enabled()) {
    ...
    if (pw->exec() == QDialog::Accepted) {
      ...
      QListWidgetItem* cdit = new QListWidgetItem(
          QIcon(":/cdrom.png"), tr("CD-ROM") + " (" + dev_name + ")",
          ui.Devices_List);
      cdit->setData(512, "cd");
    ...
    AQGraphic_Warning(tr("Warning!"), tr("Maximum CD-ROM Disk Count is 1"));
  }
}
...
void Device_Manager_Widget::Add_HDD(VM_HDD& dev, QString letter)
{
  ...
  QListWidgetItem* hdit = new QListWidgetItem(
      QIcon(":/hdd.png"), tr("HD") + letter.toUpper() + " (" + dev_name + ")",
      ui.Devices_List);
...
void Device_Manager_Widget::on_actionAdd_Device_triggered()
{
  ...
  if (Device_Window.exec() == QDialog::Accepted) {
    ...
    QListWidgetItem* devit = new QListWidgetItem(
        QIcon(":blockdevice.png"),
        Device_Window.Get_Device().Get_QEMU_Device_Name(), ui.Devices_List);
    devit->setData(512,
                   "device" + QString::number(Storage_Devices.count() - 1));
  ...
  if (ui.Devices_List->currentItem()->data(512).toString() == "fd1") {
    ...
    if (pw->exec() == QDialog::Accepted) {
      if (Floppy1 != pw->Get_Floppy()) {
        ...
        ui.Devices_List->currentItem()->setText(tr("Floppy 1") + " (" +
                                                Floppy1.Get_File_Name() + ")");
    ...
    if (pw->exec() == QDialog::Accepted) {
      if (Floppy2 != pw->Get_Floppy()) {
        ...
        ui.Devices_List->currentItem()->setText(tr("Floppy 2") + " (" +
                                                Floppy2.Get_File_Name() + ")");
    ...
    if (pw->exec() == QDialog::Accepted) {
      if (CD_ROM != pw->Get_CD_ROM()) {
        ...
        ui.Devices_List->currentItem()->setText(tr("CD-ROM") + " (" +
    ...
    if (pw->exec() == QDialog::Accepted) {
      if (HDA != pw->Get_HDD()) {
        ...
        ui.Devices_List->currentItem()->setText(tr("HDA") + " (" +
                                                HDA.Get_File_Name() + ")");
    ...
    if (pw->exec() == QDialog::Accepted) {
      if (HDB != pw->Get_HDD()) {
        ...
        ui.Devices_List->currentItem()->setText(tr("HDB") + " (" +
                                                HDB.Get_File_Name() + ")");
    ...
    if (pw->exec() == QDialog::Accepted) {
      if (HDC != pw->Get_HDD()) {
        ...
        ui.Devices_List->currentItem()->setText(tr("HDC") + " (" +
                                                HDC.Get_File_Name() + ")");
    ...
    if (pw->exec() == QDialog::Accepted) {
      if (HDD != pw->Get_HDD()) {
        ...
        ui.Devices_List->currentItem()->setText(tr("HDD") + " (" +
                                                HDC.Get_File_Name() + ")");
...
{
  int mes_ret = QMessageBox::question(this, tr("Delete?"), tr("Delete Device?"),
  ...
  if (ui.Devices_List->currentItem()->data(512).toString() == "fd1") {
    ...
    if (!found) {
      ...
      for (int ix = 0, count = 0; ix < ui.Devices_List->count(); ++ix) {
        if (ui.Devices_List->item(ix)->data(512).toString().contains(
                "device")) {
          ui.Devices_List->item(ix)->setData(512,
...
void Device_Manager_Widget::on_actionQuick_Format_triggered()
{
  int mes_ret = QMessageBox::question(
      this, tr("Format?"), tr("Format Virtual Hard Disk?"),
...
void Device_Manager_Widget::Update_Icons()
{
  ...
  if (HDA.Get_Enabled()) {
    QListWidgetItem* hdit = new QListWidgetItem(
        QIcon(":/hdd.png"), tr("HDA") + " (" + HDA.Get_File_Name() + ")",
        ui.Devices_List);
  ...
  if (HDB.Get_Enabled()) {
    QListWidgetItem* hdit = new QListWidgetItem(
        QIcon(":/hdd.png"), tr("HDB") + " (" + HDB.Get_File_Name() + ")",
        ui.Devices_List);
  ...
  if (HDC.Get_Enabled()) {
    QListWidgetItem* hdit = new QListWidgetItem(
        QIcon(":/hdd.png"), tr("HDC") + " (" + HDC.Get_File_Name() + ")",
        ui.Devices_List);
  ...
  if (HDD.Get_Enabled()) {
    QListWidgetItem* hdit = new QListWidgetItem(
        QIcon(":/hdd.png"), tr("HDD") + " (" + HDD.Get_File_Name() + ")",
        ui.Devices_List);
...
void Device_Manager_Widget::Update_HDA(bool ok)
{
  ...
  ui.Label_Connected_To->setText(tr("Image Virtual Size: ") +
                                 QString::number(HDA.Get_Virtual_Size().Size) +
                                 Get_TR_Size_Suffix(HDA.Get_Virtual_Size()) +
                                 "\n" + tr("Image On Disk Size: ") +
                                 QString::number(HDA.Get_Disk_Size().Size) +
