	...
	if (Network_Cards.count() > 0)
	{
		...
		for (int nx = 0; nx < Network_Cards.count() && nx < 8; ++nx)
		{
			...
			case VM::Net_Mode_Native_NIC:
				new QListWidgetItem("NIC " + (item_name.isEmpty() ? QString::number(++nic) : "(" + Network_Cards[nx].Get_Name() + ")"),
									ui.Items_List);
				break;

			case VM::Net_Mode_Native_User:
				new QListWidgetItem("User " + (item_name.isEmpty() ? QString::number(++user) : "(" + Network_Cards[nx].Get_Name() + ")"),
									ui.Items_List);
				break;
			...
			case VM::Net_Mode_Native_Bridge:
				new QListWidgetItem("Bridge " + (item_name.isEmpty() ? QString::number(++bridge) : "(" + Network_Cards[nx].Get_Name() + ")"),
									ui.Items_List);
				break;

			case VM::Net_Mode_Native_TAP:
				new QListWidgetItem("TAP " + (item_name.isEmpty() ? QString::number(++tap) : "(" + Network_Cards[nx].Get_Name() + ")"),
									ui.Items_List);
				break;

			case VM::Net_Mode_Native_Socket:
				new QListWidgetItem("Socket " + (item_name.isEmpty() ? QString::number(++socket) : "(" + Network_Cards[nx].Get_Name() + ")"),
									ui.Items_List);
				break;

			case VM::Net_Mode_Native_MulticastSocket:
				new QListWidgetItem("Multicast Socket " + (item_name.isEmpty() ? QString::number(++multi) : "(" + Network_Cards[nx].Get_Name() + ")"),
									ui.Items_List);
				break;

			case VM::Net_Mode_Native_VDE:
				new QListWidgetItem("VDE " + (item_name.isEmpty() ? QString::number(++vde) : "(" + Network_Cards[nx].Get_Name() + ")"),
									ui.Items_List);
				break;
...
void Network_Widget::Update_Card_Item()
{
	...
	{
	case 0:
		ui.Items_List->currentItem()->setText("NIC " + (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_NIC) : n));
		break;

	case 1:
		ui.Items_List->currentItem()->setText("User " + (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_User) : n));
		break;
	...
	case 3:
		ui.Items_List->currentItem()->setText("Bridge " + (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_Bridge) : n));
		break;

	case 4:
		ui.Items_List->currentItem()->setText("TAP " + (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_TAP) : n));
		break;

	case 5:
		ui.Items_List->currentItem()->setText("Socket " + (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_Socket) : n));
		break;

	case 6:
		ui.Items_List->currentItem()->setText("Multicast Socket " + (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_MulticastSocket) : n));
		break;

	case 7:
		ui.Items_List->currentItem()->setText("VDE " + (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_VDE) : n));
		break;
