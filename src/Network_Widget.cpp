...
bool Network_Widget::Get_Network_Cards(QList<VM_Net_Card_Native>& cards)
{
  if (ui.Items_List->count() < 1) {
    AQDebug(
        "bool Network_Widget::Get_Network_Card( QList<VM_Net_Card_Native> "
        "&cards ) const",
        "No Items");
    cards.clear();
    return true;
    ...
    for (int nx = 0; nx < Network_Cards.count() && nx < 8; ++nx) {
      ...
      switch (Network_Cards[nx].Get_Network_Type()) {
        ...
        case VM::Net_Mode_Native_Chanel:
          new QListWidgetItem("Channel " + QString::number(++chanel),
                              ui.Items_List);
          break;

        case VM::Net_Mode_Native_Bridge:
          new QListWidgetItem(
              "Bridge " + (item_name.isEmpty()
...
{
  if (ui.CB_model->count() > 0)
    ui.CB_model->clear();
  if (Card_Models_QEMU_Name.count() > 0)
    Card_Models_QEMU_Name.clear();
...
void Network_Widget::Set_Devices(const Available_Devices& devices)
{
  ...
  if (devices.PSO_Net_type_vde)
    ui.CB_Network_Type->addItem("vde");
  if (devices.PSO_Net_type_dump)
    ui.CB_Network_Type->addItem("dump");
...
{
  connect(ui.CB_Network_Type, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.CB_model, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.CH_macaddr, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_macaddr, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_name, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_name, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_hostname, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_hostname, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.Edit_port_dev, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_fd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.SB_fd, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  connect(ui.CH_ifname, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.CH_bridge, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_ifname, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.Edit_bridge, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_script, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_script, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_downscript, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_downscript, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_helper, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_helper, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_listen, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_listen, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_connect, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_connect, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_mcast, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_mcast, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_sock, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_sock, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_port, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.SB_port, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  connect(ui.CH_group, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_group, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_mode, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_mode, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_file, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_file, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_len, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.CB_len, SIGNAL(currentIndexChanged(int)), this, SIGNAL(Changed()));

  connect(ui.CH_addr, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_addr, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_vectors, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.SB_vectors, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  connect(ui.CH_net, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_net, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_host, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_host, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_restrict, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.CB_restrict, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.CH_dhcpstart, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_dhcpstart, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_dns, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_dns, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_tftp, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_tftp, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_bootfile, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_bootfile, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_smb, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_smb, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_smbserver, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_smbserver, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_sndbuf, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.SB_sndbuf, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  connect(ui.CH_vnet_hdr, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.CB_vnet_hdr, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.CH_vhost, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.CB_vhost, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.CH_vhostfd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.SB_vhostfd, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  connect(ui.CH_hostfwd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_hostfwd, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.CH_guestfwd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_guestfwd, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  // Update Name
  connect(ui.CH_name, SIGNAL(clicked()), this, SLOT(Update_Card_Item()));

  connect(ui.Edit_name, SIGNAL(textChanged(const QString&)), this,
          SLOT(Update_Card_Item()));
...
{
  disconnect(ui.CB_Network_Type, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.CB_model, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_macaddr, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_macaddr, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_name, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_name, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_hostname, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_hostname, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.Edit_port_dev, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_fd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.SB_fd, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  disconnect(ui.CH_ifname, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_ifname, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_bridge, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_bridge, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_script, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_script, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_downscript, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_downscript, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_helper, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_helper, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_listen, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_listen, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_connect, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_connect, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_mcast, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_mcast, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_sock, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_sock, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_port, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.SB_port, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  disconnect(ui.CH_group, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_group, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_mode, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_mode, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_file, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_file, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_len, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.CB_len, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_addr, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_addr, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_vectors, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.SB_vectors, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  disconnect(ui.CH_net, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_net, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_host, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_host, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_restrict, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.CB_restrict, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_dhcpstart, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_dhcpstart, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_dns, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_dns, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_tftp, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_tftp, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_bootfile, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_bootfile, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_smb, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_smb, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_smbserver, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_smbserver, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_sndbuf, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.SB_sndbuf, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  disconnect(ui.CH_vnet_hdr, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.CB_vnet_hdr, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_vhost, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.CB_vhost, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_vhostfd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.SB_vhostfd, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  disconnect(ui.CH_hostfwd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_hostfwd, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.CH_guestfwd, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_guestfwd, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  // Update Name
  disconnect(ui.CH_name, SIGNAL(clicked()), this, SLOT(Update_Card_Item()));
...
{
  if (Network_Cards.count() < 1 || ui.Items_List->count() < 1)
    return;
  ...
  if (Network_Cards.count() <= 0)
    Enable_Buttons(true, false);
  else if (Network_Cards.count() < 8)
    Enable_Buttons(true, true);

  emit Changed();
}

void Network_Widget::on_Items_List_currentItemChanged(QListWidgetItem* current,
                                                      QListWidgetItem* previous)
{
  ...
  if (cur < 0 || cur >= Network_Cards.count() ||
      prev >= Network_Cards.count()) {
    if (Network_Cards.count() <= 0)
      Enable_Buttons(true, false);
    return;
  }
  ...
  if (!Net_Card_is_Valid())
    ...
    Network_Cards[prev] = Get_Net_Card_From_Ui();
...
void Network_Widget::on_TB_Help_clicked()
{
  ...
  if (ui.CB_Network_Type->currentText() == "nic") {
    QMessageBox::information(
        this, tr("nic"),
        tr("-net nic[,macaddr=addr][,model=type][,name=name] \nCreate a new "
           "Network Interface Card and connect it to VLAN n (n = 0 is the "
           "default). The NIC is an ne2k_pci by default on the PC target. "
           "Optionally, the MAC address can be changed to addr and a name can "
           "be assigned for use in monitor commands. If no \'-net\' option is "
           "specified, a single NIC is created. Qemu can emulate several "
           "different models of network card. Valid values for type are "
           "i82551, i82557b, i82559er, ne2k_pci, ne2k_isa, pcnet, rtl8139, "
           "e1000, smc91c111, lance and mcf_fec. Not all devices are supported "
  ...
  // -net user[,hostname=name][,name=name]
    ...
    QMessageBox::information(
        this, tr("user"),
        tr("-net user[,hostname=name][,name=name] \nUse the user mode network "
           "stack which requires no administrator privilege to run. "
           "\'hostname=name\' can be used to specify the client hostname "
  ...
  // -net channel,port:dev
    ...
    QMessageBox::information(
        this, tr("channel"),
        tr("-net channel,port:dev \nForward \'user\' TCP connection to port "
           "port to character device dev"));
  }
  // -net bridge[,name=name][,br=bridge][,helper=helper]
  else if (ui.CB_Network_Type->currentText() == "bridge") {
    QMessageBox::information(
        this, tr("bridge"),
        tr("-net bridge[,name=name][,br=bridge][,helper=helper] \nConnect a "
           "host TAP network interface to a host bridge device. \nUse the "
           "network helper to configure the TAP interface and attach it to the "
  ...
  // -net tap[,name=name][,fd=h][,ifname=name][,script=file][,downscript=dfile]
    ...
    QMessageBox::information(
        this, tr("tap"),
        tr("-net "
           "tap[,name=name][,fd=h][,ifname=name][,script=file][,downscript="
           "dfile][,helper=helper] \nConnect the host TAP network interface "
           "name to VLAN n. \nUse the network script file to configure it and "
           "the network script dfile to deconfigure it. If name is not "
           "provided, the OS automatically provides one. The default network "
           ...
           "as an unprivileged user, use the network helper \'helper\' to "
           "configure the TAP interface. The default network helper executable "
           "is \'/path/to/qemu-bridge-helper\'. \n\'fd\'=h can be used to "
           "specify the handle of an already opened host TAP interface."));
  }
  // -net socket[,name=name][,fd=h][,listen=[host]:port][,connect=host:port]
    ...
    QMessageBox::information(
        ...
        tr("-net "
           ...
           "\'listen\' option. \'fd\'=h specifies an already opened TCP "
           "socket."));
  }
  // -net socket[,name=name][,fd=h][,mcast=maddr:port]
    ...
    QMessageBox::information(
        this, tr("socket"),
        tr("-net socket[,name=name][,fd=h][,mcast=maddr:port] \nCreate a VLAN "
           "n shared with another QEMU virtual machines using a UDP multicast "
           "socket, effectively making a bus for every QEMU with same "
           "multicast address maddr and port. \nNOTES: \n1. Several QEMU can "
           "be running on different hosts and share same bus (assuming correct "
           "multicast setup for these hosts). \n2. mcast support is compatible "
           "with User Mode Linux (argument \'ethN=mcast\'), see "
           "http://user-mode-linux.sf.net. \n3. Use \'fd=h\' to specify an "
  ...
  // -net vde[,name=name][,sock=socketpath][,port=n][,group=groupname][,mode=octalmode]
    ...
    QMessageBox::information(
        this, tr("vde"),
        tr("-net "
           "vde[,name=name][,sock=socketpath][,port=n][,group=groupname][,mode="
           "octalmode] \nConnect VLAN n to PORT n of a vde switch running on "
           "host and listening for incoming connections on socketpath. Use "
  ...
  // -net dump[,file=file][,len=len]
    ...
    QMessageBox::information(
        this, tr("dump"),
        tr("-net dump[,file=f][,len=n] \ndump traffic on vlan \'n\' to file "
...
void Network_Widget::on_TB_Browse_script_clicked()
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Select script"), Get_Last_Dir_Path(ui.Edit_script->text()),
...
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Select script"), Get_Last_Dir_Path(ui.Edit_downscript->text()),
      tr("All Files (*);;Scripts (*.sh)"));
...
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Select network helper"),
      Get_Last_Dir_Path(ui.Edit_helper->text()), tr("All Files (*);;"));
...
void Network_Widget::on_CB_Network_Type_currentIndexChanged(int index)
{
  ...
  // -net bridge[,name=name][,br=str][,helper=file]
  else if (ui.CB_Network_Type->currentText() == "bridge") {
    ui.CH_name->setVisible(true);
    ui.Edit_name->setVisible(true);
  ...
  // -net dump[,file=file][,len=len]
    ...
    if (ui.CB_Network_Type->count() > 0)
      AQError(
          "void Network_Widget::on_CB_Network_Type_currentIndexChanged( int "
          "index )",
          "Invalid connection type! Data: " +
              ui.CB_Network_Type->currentText());
  }
...
void Network_Widget::on_TB_Generate_New_MAC_clicked()
{
  ...
  ui.Edit_macaddr->setText(
      Network_Cards[ui.Items_List->currentRow()].Generate_MAC());
}

void Network_Widget::on_TB_Browse_bootfile_clicked()
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Boot file"), Get_Last_Dir_Path(ui.Edit_bootfile->text()),
      tr("All Files (*)"));
...
void Network_Widget::on_TB_Browse_smb_clicked()
{
  QString dirName = QFileDialog::getExistingDirectory(
      this, tr("SMB directory"), Get_Last_Dir_Path(ui.Edit_bootfile->text()));
...
void Network_Widget::Set_Net_Card_To_Ui(const VM_Net_Card_Native& card)
{
  switch (card.Get_Network_Type()) {
    ...
    default:
      AQError(
          "void Network_Widget::Set_Net_Card_To_Ui( const VM_Net_Card_Native "
          "&card )",
          "Cannot Read Network Type! Use Default: User Mode");
      ui.CB_Network_Type->setCurrentIndex(1);
      break;
...
{
  bool u_macaddr, u_name, u_hostname, u_port_dev, u_ifname, u_script,
      u_downscript, u_bridge, u_helper, u_listen, u_connect, u_mcast, u_sock,
      u_group, u_mode, u_file, u_len;

  u_macaddr = u_name = u_hostname = u_port_dev = u_ifname = u_script =
      u_downscript = u_bridge = u_helper = u_listen = u_connect = u_mcast =
          u_sock = u_group = u_mode = u_file = u_len = false;
  ...
  switch (ui.CB_Network_Type->currentIndex()) {
    ...
    // -net tap[,name=name][,fd=h][,ifname=name][,script=file][,downscript=dfile][,helper=helper]
    case 4:
  ...
  if (u_name && ui.CH_name->isChecked()) {
    ...
    if (!rx.match(ui.Edit_name->text()).hasMatch()) {
      AQGraphic_Warning(
          tr("Error!"),
          tr("Invalid name! Valid characters are:: a-z A-Z 0-9 _ -"));
      return false;
    }
  }
  ...
  if (u_hostname && ui.CH_hostname->isChecked()) {
    ...
    if (!rx.match(ui.Edit_hostname->text()).hasMatch()) {
      AQGraphic_Warning(
          tr("Error!"),
          tr("Invalid Hostname! Valid characters are:: a-z A-Z 0-9 _ -"));
      return false;
    }
  }
...
{
  int retVal = QMessageBox::critical(
      this, tr("Error!"),
      tr("%1 File \"%2\" doesn't Exist! Continue Without this File?")
          .arg(name)
          .arg(path),
...
void Network_Widget::Update_Card_Item()
{
  ...
  switch (ui.CB_Network_Type->currentIndex()) {
    case 0:
      ui.Items_List->currentItem()->setText(
          "NIC " +
          (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_NIC) : n));
      break;

    case 1:
      ui.Items_List->currentItem()->setText(
          "User " + (item_name.isEmpty()
                         ? Get_Items_Count(VM::Net_Mode_Native_User)
                         : n));
      break;

    case 2:
      ui.Items_List->currentItem()->setText(
          "Channel " + Get_Items_Count(VM::Net_Mode_Native_Chanel));
      break;

    case 3:
      ui.Items_List->currentItem()->setText(
          "Bridge " + (item_name.isEmpty()
                           ? Get_Items_Count(VM::Net_Mode_Native_Bridge)
                           : n));
      break;
    ...
    case 4:
      ui.Items_List->currentItem()->setText(
          "TAP " +
          (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_TAP) : n));
      break;

    case 5:
      ui.Items_List->currentItem()->setText(
          "Socket " + (item_name.isEmpty()
                           ? Get_Items_Count(VM::Net_Mode_Native_Socket)
                           : n));
      break;
    ...
    case 6:
      ui.Items_List->currentItem()->setText(
          "Multicast Socket " +
          (item_name.isEmpty()
               ? Get_Items_Count(VM::Net_Mode_Native_MulticastSocket)
               : n));
      break;
    ...
    case 7:
      ui.Items_List->currentItem()->setText(
          "VDE " +
          (item_name.isEmpty() ? Get_Items_Count(VM::Net_Mode_Native_VDE) : n));
      break;

    case 8:
      ui.Items_List->currentItem()->setText(
          "Dump " + Get_Items_Count(VM::Net_Mode_Native_Dump));
      break;
...
QString Network_Widget::Get_Items_Count(VM::Network_Mode_Native type)
{
  ...
  for (int ix = 0; ui.Items_List->currentRow() >= ix; ix++) {
    if (Network_Cards[ix].Get_Network_Type() == type)
      count++;
  }
