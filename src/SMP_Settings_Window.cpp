...
SMP_Settings_Window::SMP_Settings_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);
}
...
void SMP_Settings_Window::Set_Values(const VM::SMP_Options& smp,
                                     unsigned short PSO_SMP_Count,
                                     bool PSO_SMP_Cores, bool PSO_SMP_Threads,
                                     bool PSO_SMP_Sockets, bool PSO_SMP_MaxCPUs)
{
  Backup_SMP = smp;
...
{
  if (QDialog::Accepted == r) {  // FIXME Calculate Max CPU Count
    QDialog::done(r);
    return;
  }
  else  //cancel
