...
Ports_Tab_Widget::Ports_Tab_Widget(QWidget* parent) : QWidget(parent)
{
  ui.setupUi(this);
...
void Ports_Tab_Widget::Set_Serial_Ports(const QList<VM_Port>& list)
{
  for (int ix = 0; ix < list.count(); ix++) {
    ...
    QTableWidgetItem* newItem =
        new QTableWidgetItem(Get_Port_Info(Serial_Ports[Index]));
...
void Ports_Tab_Widget::Set_Parallel_Ports(const QList<VM_Port>& list)
{
  for (int ix = 0; ix < list.count(); ix++) {
    ...
    QTableWidgetItem* newItem =
        new QTableWidgetItem(Get_Port_Info(Parallel_Ports[Index]));
    newItem->setData(Qt::UserRole, Index);
    ui.Ports_Table->setItem(ui.Ports_Table->rowCount() - 1, 1, newItem);
...
void Ports_Tab_Widget::Set_USB_Ports(const QList<VM_USB>& list)
{
  for (int ix = 0; ix < list.count(); ix++) {
    ...
    QTableWidgetItem* newItem =
        new QTableWidgetItem(Get_USB_Port_Info(USB_Ports[Index]));
    newItem->setData(Qt::UserRole, Index);
    ui.Ports_Table->setItem(ui.Ports_Table->rowCount() - 1, 1, newItem);
...
void Ports_Tab_Widget::Clear_Old_Ports()
{
  ...
  for (int rx = 0; rx < ui.Ports_Table->rowCount();)
    ui.Ports_Table->removeRow(rx);
...
QString Ports_Tab_Widget::Get_USB_Port_Info(const VM_USB& port)
{
  if (port.Get_Use_Host_Device() == false) {
    ...
    if (port.Get_Bus().isEmpty() || port.Get_Addr().isEmpty() ||
        port.Get_DevPath().isEmpty()) {
      return tr("%1 %2 (Vendor ID: %3 Product ID: %4)")
          .arg(port.Get_Manufacturer_Name())
          .arg(port.Get_Product_Name())
          .arg(port.Get_Vendor_ID())
          .arg(port.Get_Product_ID());
    }
    else {
      return tr("%1 %2 (Bus.Addr.Path: %3.%4.%5)")
          .arg(port.Get_Manufacturer_Name())
          .arg(port.Get_Product_Name())
          .arg(port.Get_Bus())
          .arg(port.Get_Addr())
...
void Ports_Tab_Widget::on_TB_Add_Serial_Port_clicked()
{
  if (Serial_Ports.count() >= 4) {
    AQGraphic_Warning(tr("Warning"),
                      tr("You can add no more than 4 COM ports"));
  }
  else {
    ...
    if (add_port_win->exec() == QDialog::Accepted) {
      ...
      QTableWidgetItem* newItem =
          new QTableWidgetItem(Get_Port_Info(add_port_win->Get_Port()));
      newItem->setData(Qt::UserRole, Index);
      ui.Ports_Table->setItem(ui.Ports_Table->rowCount() - 1, 1, newItem);
...
void Ports_Tab_Widget::on_TB_Add_Parallel_Port_clicked()
{
  if (Parallel_Ports.count() >= 3) {
    AQGraphic_Warning(tr("Warning"),
                      tr("You can add no more than 3 LPT ports"));
  }
  else {
    Add_Port_Window* add_port_win = new Add_Port_Window(this);
    add_port_win->Set_Port(VM_Port());
...
void Ports_Tab_Widget::on_TB_Add_USB_Port_clicked()
{
  ...
  if (add_usb_win->exec() == QDialog::Accepted) {
    ...
    QTableWidgetItem* newItem =
        new QTableWidgetItem(Get_USB_Port_Info(add_usb_win->Get_Port()));
    newItem->setData(Qt::UserRole, Index);
    ui.Ports_Table->setItem(ui.Ports_Table->rowCount() - 1, 1, newItem);
...
void Ports_Tab_Widget::on_TB_Edit_Port_clicked()
{
  if (ui.Ports_Table->currentRow() < 0) {
    ...
    // Port Type
    QString type = ui.Ports_Table->item(ui.Ports_Table->currentRow(), 0)
                       ->data(Qt::UserRole)
                       .toString();
    int ix = ui.Ports_Table->item(ui.Ports_Table->currentRow(), 1)
                 ->data(Qt::UserRole)
                 .toInt();
...
void Ports_Tab_Widget::on_TB_Delete_Port_clicked()
{
  if (ui.Ports_Table->currentRow() < 0) {
    ...
    if (type == "COM")
      Serial_Ports.remove(ix);
    else if (type == "LPT")
      Parallel_Ports.remove(ix);
    else if (type == "USB")
      USB_Ports.remove(ix);
    else {
