...
Properties_Window::Properties_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);

  HDD_Info = new HDD_Image_Info();

  connect(ui.CB_FD_Devices, SIGNAL(editTextChanged(QString)), this,
          SLOT(on_Button_Update_Info_clicked()));

  connect(ui.CB_CDROM_Devices, SIGNAL(editTextChanged(QString)), this,
          SLOT(on_Button_Update_Info_clicked()));

  connect(ui.Edit_HDD_Image_Path, SIGNAL(textChanged(QString)), this,
          SLOT(on_Button_Update_Info_clicked()));
}
...
void Properties_Window::Set_Floppy(const VM_Storage_Device& fd,
                                   const QString& name)
{
  PW_Storage = fd;
...
void Properties_Window::Set_CD_ROM(const VM_Storage_Device& cd,
                                   const QString& name)
{
  PW_Storage = cd;
  ...
  if (cd_list.count() < 1) {
    AQDebug(
        "void Properties_Window::Set_CD_ROM( const VM_CDROM &cd, const QString "
        "&name )",
...
void Properties_Window::Set_Current_Machine_Devices(
    const Available_Devices* dev)
{
...
void Properties_Window::done(int r)
{
  if (r == QDialog::Accepted) {
    if (ui.GB_Floppy->isEnabled()) {
      ...
      AQError("void Properties_Window::done(int)", "Default Section!");
      return;
    }
  }
  QDialog::done(r);
...
{
  if (ui.GB_Floppy->isEnabled()) {
    VM_Storage_Device* fd =
        new VM_Storage_Device(true, ui.CB_FD_Devices->lineEdit()->text());

    if (ui.CB_FD_Devices->lineEdit()->text().isEmpty()) {
      ui.Label_Info->setText(tr("Image Size: ") + QString::number(0) +
                             tr("Kb"));
      return;
    }
    ...
    if (size_in_bytes <= 0)
      ...
      ui.Label_Info->setText(tr("Image Size: ") +
                             QString::number((int)size_in_bytes / 1024.0) +
                             tr("Kb"));

    delete fd;
  }
  else if (ui.GB_CDROM->isEnabled()) {
    VM_Storage_Device* cd =
        new VM_Storage_Device(true, ui.CB_CDROM_Devices->lineEdit()->text());
    ...
    if (size_in_bytes <= 0)
      ...
      ui.Label_Info->setText(
          tr("Image Size: ") +
          QString::number((float)size_in_bytes / 1024.0 / 1024.0, 'f', 2) +
          tr("Mb"));
...
void Properties_Window::on_TB_FD_Image_Browse_clicked()
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Open Floppy Image File"),
...
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Open CD\\DVD-ROM Image File"),
...
void Properties_Window::Update_HDD(bool ok)
{
  ...
  ui.Label_Info->setText(
      tr("Format: ") + PW_HDD.Get_Image_Format() + tr(" Virtual Size: ") +
      QString::number(PW_HDD.Get_Virtual_Size().Size) +
      Get_TR_Size_Suffix(PW_HDD.Get_Virtual_Size()) + tr("\nOn Disk Size: ") +
      QString::number(PW_HDD.Get_Disk_Size().Size) +
      Get_TR_Size_Suffix(PW_HDD.Get_Disk_Size()) + tr(" Cluster Size: ") +
      QString::number(PW_HDD.Get_Cluster_Size()));
}
...
void Properties_Window::on_TB_FD_Advanced_Settings_clicked()
{
  ...
  if (win->exec() == QDialog::Accepted) {
    ...
    if (PW_Storage.Get_Native_Device().Get_File_Path() !=
        ui.CB_FD_Devices->currentText())
      ui.CB_FD_Devices->setEditText(
          PW_Storage.Get_Native_Device().Get_File_Path());
...
void Properties_Window::on_TB_CDROM_Advanced_Settings_clicked()
{
  ...
  if (win->exec() == QDialog::Accepted) {
    ...
    if (PW_Storage.Get_Native_Device().Get_File_Path() !=
        ui.CB_CDROM_Devices->currentText())
...
void Properties_Window::on_TB_HDD_Advanced_Settings_clicked()
{
  ...
  if (win->exec() == QDialog::Accepted) {
    ...
    if (PW_HDD.Get_Native_Device().Get_File_Path() !=
        ui.Edit_HDD_Image_Path->text())
      ui.Edit_HDD_Image_Path->setText(
          PW_HDD.Get_Native_Device().Get_File_Path());
