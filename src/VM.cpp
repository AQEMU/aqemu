  ...
  if (this->Icon_Path == vm.Get_Icon_Path() && this->Computer_Type == vm.Get_Computer_Type() &&
    ...
    if (Shared_Folders.count() == vm.Get_Shared_Folders_List().count()) {
      for (int sx = 0; sx < Shared_Folders.count(); ++sx) {
        if (Shared_Folders[sx] != vm.Get_Shared_Folders_List()[sx])
          return false;
      }
    }
    else
    ...
    if (Network_Cards_Native.count() == tmp_card_native.count()) {
      for (int nx = 0; nx < tmp_card_native.count(); ++nx) {
        if (Network_Cards_Native[nx] != tmp_card_native[nx])
          return false;
      }
    }
    else
    ...
    if (Serial_Ports.count() == vm.Get_Serial_Ports().count()) {
      for (int sx = 0; sx < Serial_Ports.count(); ++sx) {
        if (Serial_Ports[sx] != vm.Get_Serial_Ports()[sx])
          return false;
      }
    }
    else
    ...
    if (Parallel_Ports.count() == vm.Get_Parallel_Ports().count()) {
      for (int px = 0; px < Parallel_Ports.count(); ++px) {
        if (Parallel_Ports[px] != vm.Get_Parallel_Ports()[px])
          return false;
      }
    }
    else
    ...
    if (USB_Ports.count() == vm.Get_USB_Ports().count()) {
      for (int ux = 0; ux < USB_Ports.count(); ++ux) {
        if (USB_Ports[ux] != vm.Get_USB_Ports()[ux])
          return false;
      }
    }
    else
...
bool Virtual_Machine::Create_VM_File(const QString& file_name, bool template_mode)
{
  ...
  if (file_name.isEmpty()) {
    AQError(
        "bool Virtual_Machine::Create_VM_File( const QString &file_name, bool "
        "template_mode )",
        "File Name is Empty!");
    return false;
  }
  ...
  if (template_mode && !(Template_Opts & Create_Template_Window::Template_Save_Network)) {
    ...
    for (qsizetype nx = 0; nx < Network_Cards_Native.count(); nx++) {
      ...
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          Network_Cards_Native[nx].Use_File_Descriptor() ? "yes" : "no");
      Sec_Element.appendChild(Dom_Text);

      Sec_Element = New_Dom_Document.createElement("File_Descriptor");
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          QString::number(Network_Cards_Native[nx].Get_File_Descriptor()));
      Sec_Element.appendChild(Dom_Text);
      ...
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          Network_Cards_Native[nx].Use_Interface_Name() ? "yes" : "no");
      Sec_Element.appendChild(Dom_Text);
      ...
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          Network_Cards_Native[nx].Use_TUN_TAP_Script() ? "yes" : "no");
      Sec_Element.appendChild(Dom_Text);
      ...
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          Network_Cards_Native[nx].Use_TUN_TAP_Down_Script() ? "yes" : "no");
      Sec_Element.appendChild(Dom_Text);
      ...
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          Network_Cards_Native[nx].Use_Bridge_Helper() ? "yes" : "no");
      Sec_Element.appendChild(Dom_Text);
    ...
    for (int rx = 0; rx < Network_Redirections.count(); rx++) {
      ...
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          QString::number(Network_Redirections[rx].Get_Host_Port()));
      Sec_Element.appendChild(Dom_Text);
      ...
      Dom_Element.appendChild(Sec_Element);
      Dom_Text = New_Dom_Document.createTextNode(
          QString::number(Network_Redirections[rx].Get_Guest_Port()));
      Sec_Element.appendChild(Dom_Text);
...
bool Virtual_Machine::Load_VM(const QString& file_name)
{
  ...
  if (!VM_File.open(QFile::ReadOnly | QFile::Text)) {
    ...
    if (!VM_Dom_Document.setContent(&VM_File, true, &Error_String, &Error_Line, &Error_Column)) {
      ...
      if (template_mode) {
        // Template Options
        Template_Opts = (Create_Template_Window::Template_Options)Child_Element
                            .firstChildElement("Template_Options")
                            .text()
                            .toInt();
      }
      ...
      if (Computer_Type == "qemu-system-x86")
      ...
      if (load_boot_order_setcton &&
        ...
        for (int bx = 0; bx < bootOrderCount; bx++) {
          ...
          if (bootType == "FDA")
            tmpBootDev.Type = VM::Boot_From_FDA;
          else if (bootType == "FDB")
            tmpBootDev.Type = VM::Boot_From_FDB;
          else if (bootType == "CDROM")
            tmpBootDev.Type = VM::Boot_From_CDROM;
          else if (bootType == "HDD")
            tmpBootDev.Type = VM::Boot_From_HDD;
          else if (bootType == "Network1")
            tmpBootDev.Type = VM::Boot_From_Network1;
          else if (bootType == "Network2")
            tmpBootDev.Type = VM::Boot_From_Network2;
          else if (bootType == "Network3")
            tmpBootDev.Type = VM::Boot_From_Network3;
          else if (bootType == "Network4")
            tmpBootDev.Type = VM::Boot_From_Network4;
          else {
            AQError("bool Virtual_Machine::Load_VM( const QString &file_name )",
          ...
          if (busAddrList.count() != 2) {
            AQError("bool Virtual_Machine::Load_VM( const QString &file_name )",
                    "Cannot parse obsolete USB Bus.Addr string! Data: " +
                        Second_Element.firstChildElement("BusAddr").text());
          }
          else {
...
VM_Native_Storage_Device Virtual_Machine::Load_VM_Native_Storage_Device(
...
QStringList Virtual_Machine::Build_QEMU_Args()
{
  ...
  // Win2000 Hack
  if (Current_Emulator_Devices.PSO_Win2K_Hack && Win2K_Hack)
    Args << "-win2k-hack";
  ...
  // No ACPI
  if (Current_Emulator_Devices.PSO_No_ACPI && ACPI == false)
    Args << "-no-acpi";
  ...
  if (FD0.Get_Enabled()) {
    if (FD0.Get_Native_Mode()) {
      ...
      StorageArgs << Build_Native_Device_Args(FD0.Get_Native_Device(),
  ...
  if (FD1.Get_Enabled()) {
    if (FD1.Get_Native_Mode()) {
      ...
      StorageArgs << Build_Native_Device_Args(FD1.Get_Native_Device(),
                                              Build_QEMU_Args_for_Tab_Info);
    }
    else {
  ...
  if (CD_ROM.Get_Enabled()) {
    if (CD_ROM.Get_Native_Mode()) {
      ...
      StorageArgs << Build_Native_Device_Args(CD_ROM.Get_Native_Device(),
                                              Build_QEMU_Args_for_Tab_Info);
    }
    else {
  ...
  if (HDA.Get_Enabled()) {
    if (HDA.Get_Native_Mode()) {
      ...
      StorageArgs << Build_Native_Device_Args(HDA.Get_Native_Device(),
                                              Build_QEMU_Args_for_Tab_Info);
    }
    else {
  ...
  if (HDB.Get_Enabled()) {
    if (HDB.Get_Native_Mode()) {
      ...
      StorageArgs << Build_Native_Device_Args(HDB.Get_Native_Device(),
                                              Build_QEMU_Args_for_Tab_Info);
    }
    else {
  ...
  if (HDC.Get_Enabled()) {
    if (HDC.Get_Native_Mode()) {
      ...
      StorageArgs << Build_Native_Device_Args(HDC.Get_Native_Device(),
                                              Build_QEMU_Args_for_Tab_Info);
    }
    else {
  ...
  if (HDD.Get_Enabled()) {
    if (HDD.Get_Native_Mode()) {
      ...
      StorageArgs << Build_Native_Device_Args(HDD.Get_Native_Device(),
                                              Build_QEMU_Args_for_Tab_Info);
    }
    else {
  ...
  // Storage Devices
  if (Current_Emulator_Devices.PSO_Drive && Storage_Devices.count() > 0) {
  ...
  if (Current_Emulator_Devices.PSO_Boot_Order) {
    ...
    if (bootDevCount > 1) {
      ...
      for (int ix = 0; ix < Boot_Order_List.count(); ix++) {
        if (Boot_Order_List[ix].Enabled) {
          if (Boot_Order_List[ix].Type == VM::Boot_From_FDA)
            bootStr += "a";
          else if (Boot_Order_List[ix].Type == VM::Boot_From_FDB)
            bootStr += "b";
          else if (Boot_Order_List[ix].Type == VM::Boot_From_HDD)
            bootStr += "c";
          else if (Boot_Order_List[ix].Type == VM::Boot_From_CDROM)
            bootStr += "d";
          else if (Boot_Order_List[ix].Type == VM::Boot_From_Network1)
            bootStr += "n-1";
          else if (Boot_Order_List[ix].Type == VM::Boot_From_Network2)
            bootStr += "n-2";
          else if (Boot_Order_List[ix].Type == VM::Boot_From_Network3)
            bootStr += "n-3";
          else if (Boot_Order_List[ix].Type == VM::Boot_From_Network4)
            bootStr += "n-4";
        }
      }
      ...
      if (Boot_Order_List[onceBootDeviceIndex].Type == VM::Boot_From_FDA)
        bootStr = "a";
      else if (Boot_Order_List[onceBootDeviceIndex].Type == VM::Boot_From_FDB)
        bootStr = "b";
      else if (Boot_Order_List[onceBootDeviceIndex].Type == VM::Boot_From_HDD)
        bootStr = "c";
    ...
    if (Use_Native_Network()) {
      for (int nc = 0; nc < Network_Cards_Native.count(); nc++) {
        ...
        switch (Network_Cards_Native[nc].Get_Network_Type()) {
          ...
          case VM::Net_Mode_Native_VDE:
            if (!Current_Emulator_Devices.PSO_Net_type_vde)
              continue;
            nic_str += "vde";
          ...
          case VM::Net_Mode_Native_Dump:
            if (!Current_Emulator_Devices.PSO_Net_type_dump)
              continue;
            nic_str += "dump";
        ...
        if (Network_Cards_Native[nc].Use_Bridge_Name() && u_bridge &&
            Current_Emulator_Devices.PSO_Net_bridge)
        ...
        if (Network_Cards_Native[nc].Use_Listen() && u_listen &&
            Current_Emulator_Devices.PSO_Net_listen)
          nic_str += ",listen=" + Network_Cards_Native[nc].Get_Listen();

        if (Network_Cards_Native[nc].Use_Connect() && u_connect &&
            Current_Emulator_Devices.PSO_Net_connect)
          nic_str += ",connect=" + Network_Cards_Native[nc].Get_Connect();

        if (Network_Cards_Native[nc].Use_MCast() && u_mcast &&
        ...
        if (Network_Cards_Native[nc].Use_File() && u_file &&
            Current_Emulator_Devices.PSO_Net_file) {
          if (Build_QEMU_Args_for_Script_Mode)
        ...
        if (Network_Cards_Native[nc].Use_Tftp() && u_tftp &&
            Current_Emulator_Devices.PSO_Net_tftp) {
          if (Build_QEMU_Args_for_Script_Mode)
        ...
        if (Network_Cards_Native[nc].Use_HostFwd() && u_hostfwd &&
            Current_Emulator_Devices.PSO_Net_hostfwd)
          nic_str += ",hostfwd=" + Network_Cards_Native[nc].Get_HostFwd();

        if (Network_Cards_Native[nc].Use_GuestFwd() && u_guestfwd &&
  ...
  if (!TFTP_Prefix.isEmpty()) {
    if (Build_QEMU_Args_for_Script_Mode)
      Args << "-net"
           << "user,tftp=" + QString("\"") + TFTP_Prefix + "\"";
    else
      Args << "-net"
           << "user,tftp=" + TFTP_Prefix;
  }
  ...
  if (!SMB_Directory.isEmpty()) {
    if (Build_QEMU_Args_for_Script_Mode)
      Args << "-net"
           << "user,smb=" + QString("\"") + SMB_Directory + "\"";
    else
      Args << "-net"
           << "user,smb=" + SMB_Directory;
  }
  ...
  for (int ix = 0; ix < Parallel_Ports.count(); ix++) {
    if (Parallel_Ports[ix].Get_Port_Redirection() == VM::PR_Default)
      continue;
    ...
    switch (device.Get_Interface()) {
      ...
      default:
        AQError(
            ...
            "VM_Native_Storage_Device device, bool "
...
bool Virtual_Machine::Start_impl()
{
    ...
    if (!lsmod.waitForFinished(1000)) {
      ...
      if (all_mod.isEmpty()) {
        ...
        if (kvm_intel_mod.match(all_mod).hasMatch())
          kvm_ok = true;
        else if (kvm_amd_mod.match(all_mod).hasMatch())
          kvm_ok = true;
        ...
        if (!kvm_ok) {
          int retVal = QMessageBox::question(
              ...
              tr("KVM Kernel Module Not Loaded!\n"
                 "To load this Module, Enter in Terminal with root privileges: "
                 "\"modprobe kvm-intel\". "
  ...
  if (Load_Mode) {
    connect(this, SIGNAL(Ready_StdOut(const QString&)), this,
            SLOT(Started_Booting(const QString&)));
...
void Virtual_Machine::Pause()
{
  switch (State) {
    ...
    default:
      AQDebug("void Virtual_Machine::Pause()", "Default Section!");
      break;
  }
}
...
bool Virtual_Machine::Take_Screenshot(const QString& file_name, int width, int height)
{
  if (!(State == VM::VMS_Running || State == VM::VMS_Pause)) {
    AQError(
        "bool Virtual_Machine::Take_Screenshot( const QString &file_name, int "
        "width, int height )",
...
const Available_Devices* Virtual_Machine::Get_Current_Emulator_Devices() const
{
  if (Current_Emulator_Devices.System.QEMU_Name.isEmpty()) {
    AQError(
        "const Virtual_Machine::Available_Devices "
        "*Get_Current_Emulator_Devices() const",
...
QString Virtual_Machine::Get_Current_Emulator_Binary_Path(const QString& names) const
{
  ...
  if (bin_list.count() <= 0 || nl.count() <= 0) {
    AQError(
        "QString Virtual_Machine::Get_Current_Emulator_Binary_Path( const "
        "QString &names ) const",
...
void Virtual_Machine::Set_VM_Network_Card(int index, const VM_Net_Card& nc)
{
  if (index >= 0 && index <= 8 && index < Network_Cards.count()) {
    ...
    AQError(
        "void Virtual_Machine::Set_VM_Network_Card( int index, VM_Net_Card *nc "
        ")",
        "Invalid Index!");
  }
}
...
const VM_Redirection& Virtual_Machine::Get_Network_Redirection(int index) const
{
  if (index >= 0 && index < Network_Redirections.count()) {
    ...
    AQError(
        "VM_Redirection *Virtual_Machine::Get_Network_Redirection( int index ) "
        "const",
        "Invalid Index!");
...
{
  AQDebug("void Virtual_Machine::QEMU_Started()", "QEMU Start");
  ...
  if (!Load_Mode) {
    AQDebug("void Virtual_Machine::QEMU_Started()", "emit Loading_Complete()");
    emit Loading_Complete();
  }
...
void Virtual_Machine::QEMU_Finished(int exitCode, QProcess::ExitStatus exitStatus)
{
  ...
  if (USB_Ports.count() > 0) {
    foreach (VM_USB usb_dev, USB_Ports)
      System_Info::Delete_From_Used_USB_List(usb_dev);
  }
...
class Setting_Or_Preview_Helper {
 public:
  Setting_Or_Preview_Helper(bool _preview) { preview = _preview; }
...
#define INFO_TEXT_INDENT 25
  ...
  if (info_mode != 0 && info_mode < 3) {
    ...
    if (calculateContrast(qApp->palette().color(QPalette::Window),
                          qApp->palette().color(QPalette::Link)) > 3.0)
  ...
  if ((Settings.value("Info/RTC_TD_Hack", "no").toString() == "yes" ||
    ...
    if (Settings.value("Info/Init_Graphical_Mode", "no").toString() == "yes" &&
      ...
      cell = table->cellAt(table->rows() - 1, 2);
      cell_cursor = cell.firstCursorPosition();
      cell_cursor.insertText(QString("%1x%2x%3")
                                 .arg(Get_Init_Graphic_Mode().Get_Width())
                                 .arg(Get_Init_Graphic_Mode().Get_Height())
                                 .arg(Get_Init_Graphic_Mode().Get_Depth()),
                             format);
      table->insertRows(table->rows(), 1);
    }
  }
