...
void Emulator_Options_Window::on_Button_Find_clicked()
{
  ...
  while (ui.Table_Systems->rowCount() > 0)
    ui.Table_Systems->removeRow(0);
  ...
  if (r == QDialog::Accepted) {
    ...
    if (!Name_Valid(ui.Edit_Name->text())) {
      AQGraphic_Warning(tr("Error!"),
                        tr("This emulator name is already used!"));
      return;
    }
    ...
    for (int ix = 0; ix < ui.Table_Systems->rowCount(); ix++) {
      bin_files[ui.Table_Systems->item(ix, 0)->text()] =
          ui.Table_Systems->item(ix, 1)->text();
    }
    Current_Emulator.Set_Binary_Files(bin_files);

    // Check version
    if (ui.RB_Check_Version->isChecked())
      Current_Emulator.Set_Check_Version(true);
    else
      Current_Emulator.Set_Check_Version(false);

    // Check available options
    if (ui.RB_Check_Options->isChecked())
      Current_Emulator.Set_Check_Available_Options(true);
    else
      Current_Emulator.Set_Check_Available_Options(false);

    // Force version
    if (ui.RB_Force_Version->isChecked())
      Current_Emulator.Set_Force_Version(true);
    else
      Current_Emulator.Set_Force_Version(false);

    // Version
    Current_Emulator.Set_Version(
        String_To_Emulator_Version(ui.CB_Version->currentText()));
...
void Emulator_Options_Window::Set_Emulator(const Emulator& emul)
{
  ...
  // Version
  if (Current_Emulator.Get_Check_Version())
    ui.RB_Check_Version->setChecked(true);  // Check version
  else if (Current_Emulator.Get_Check_Available_Options())
    ui.RB_Check_Options->setChecked(true);  // Check available options
  else if (Current_Emulator.Get_Force_Version())
    ui.RB_Force_Version->setChecked(true);  // Force version
  else
    ui.RB_Save_Options->setChecked(
        true);  // Use saved emulator available options
  ...
  // Add new
  for (QMap<QString, QString>::const_iterator iter = bin_files.constBegin();
       iter != bin_files.constEnd(); iter++) {
    ui.Table_Systems->insertRow(ui.Table_Systems->rowCount());
...
void Emulator_Options_Window::Set_All_Emulators_Names(
...
void Emulator_Options_Window::on_Table_Systems_itemDoubleClicked(
    QTableWidgetItem* item)
{
  QString binName = QFileDialog::getOpenFileName(
      this, tr("Select QEMU Binary File"),
      Get_Last_Dir_Path(ui.Edit_Path_to_Dir->text()), tr("All Files (*)"));

  if (!binName.isEmpty()) {
    if (ui.Table_Systems->currentRow() < 0)
      return;

    ui.Table_Systems->item(ui.Table_Systems->currentRow(), 1)
        ->setText(QDir::toNativeSeparators(binName));
...
bool Emulator_Options_Window::Name_Valid(const QString& name)
{
  ...
  for (int fx = 0; fx < All_Emulators_Names.count(); fx++) {
    if (All_Emulators_Names[fx] == name && Current_Emulator.Get_Name() != name)
      return false;
  }
...
void Emulator_Options_Window::Update_Emulator()
{
  if (Update_Info) {
    if (ui.RB_Save_Options->isChecked() ||  // Update emulator bin files info?
      ...
      for (int ix = 0; ix < ui.Table_Systems->rowCount(); ix++) {
        ...
        Available_Devices tmpDev = System_Info::Get_Emulator_Info(
            ui.Table_Systems->item(ix, 1)->text(), &ok,
            Current_Emulator.Get_Version(),
            ui.Table_Systems->item(ix, 0)->text());

        if (ok)
          devList[ui.Table_Systems->item(ix, 0)->text()] = tmpDev;
        else
          AQGraphic_Warning(tr("Error!"),
                            tr("Cannot get emulator info! For file: %1")
                                .arg(ui.Table_Systems->item(ix, 1)->text()));
      }
    ...
    else if (ui.RB_Check_Version->isChecked())  // Update version
    {
      ...
      for (int ix = 0; ix < ui.Table_Systems->rowCount(); ix++) {
        if (QFile::exists(ui.Table_Systems->item(ix, 1)->text()))
          emul_version = System_Info::Get_Emulator_Version(
              ui.Table_Systems->item(ix, 1)->text());
        else
          continue;
