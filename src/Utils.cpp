                          ...
                          VM::Device_Size size, bool verbose)
{
  ...
  if (!qemu_img.waitForFinished(10000)) {
    ...
    if (err.size() > 0) {
      AQGraphic_Error(
          "bool Create_New_HDD_Image( bool encrypted, const QString "
          "&base_image,"
          "const QString &file_name, const QString &format, VM::Device_Size "
          "size, bool verbose )",
          QObject::tr("Error!"),
...
bool Update_Emulators_List()
{
  ...
  for (int ex = 0; ex < emulFiles.count(); ++ex) {
    ...
    if (tmp.Get_Check_Available_Options()) {
      ...
      for (QMap<QString, QString>::const_iterator it = tmpBinFiles.constBegin();
  ...
  if (Emulators_List.isEmpty()) {
    AQWarning("bool Update_Emulators_List()", "No emulators loaded!");
    return false;
  }
...
const Emulator& Get_Default_Emulator()
{
  if (Emulators_List.count() <= 0)
    AQError("const Emulator &Get_Default_Emulator()", "Emulator Count == 0");
  else {
  ...
  AQWarning("const Emulator &Get_Default_Emulator()", "Cannot Find!");
  return Empty_Emul;
}
