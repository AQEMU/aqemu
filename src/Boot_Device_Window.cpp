...
Boot_Device_Window::Boot_Device_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);
}
  ...
  for (int ix = 0; ix < ui.Boot_Devices_List->topLevelItemCount(); ix++) {
    ...
    if (item != NULL) {
      ...
      if (item->checkState(0) == Qt::Checked)
        ...
        tmpDev.Enabled = false;

      QString typeStr = item->data(0, Qt::UserRole).toString();

      if (typeStr == "FDA")
        tmpDev.Type = VM::Boot_From_FDA;
      else if (typeStr == "FDB")
        tmpDev.Type = VM::Boot_From_FDB;
      else if (typeStr == "CDROM")
        tmpDev.Type = VM::Boot_From_CDROM;
      else if (typeStr == "HDD")
        tmpDev.Type = VM::Boot_From_HDD;
      else if (typeStr == "Network1")
        tmpDev.Type = VM::Boot_From_Network1;
      else if (typeStr == "Network2")
        tmpDev.Type = VM::Boot_From_Network2;
      else if (typeStr == "Network3")
        tmpDev.Type = VM::Boot_From_Network3;
      else if (typeStr == "Network4")
        tmpDev.Type = VM::Boot_From_Network4;
      else {
...
void Boot_Device_Window::setData(const QList<VM::Boot_Order>& list)
{
  ...
  for (int ix = 0; ix < list.count(); ix++) {
    ...
    switch (list[ix].Type) {
      ...
      default:
        AQWarning(
            "void Boot_Device_Window::setData( QList<VM::Boot_Order> &list )",
...
void Boot_Device_Window::on_TB_Up_clicked()
{
  ...
  if (index < 1 || index > ui.Boot_Devices_List->topLevelItemCount())
    return;
...
{
  int index = Get_Current_Index();
  if (index < 0 || index > ui.Boot_Devices_List->topLevelItemCount() - 2)
    return;
...
int Boot_Device_Window::Get_Current_Index()
{
  ...
  for (int ix = 0; ix < ui.Boot_Devices_List->topLevelItemCount(); ++ix) {
    if (ui.Boot_Devices_List->topLevelItem(ix) == currentItem)
      return ix;
  }
