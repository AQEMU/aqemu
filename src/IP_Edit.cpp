...
IP_Edit::IP_Edit(QWidget* parent) : QFrame(parent)
{
  ...
  for (linesIterator = lines.begin(); linesIterator != lines.end();
       ++linesIterator) {
  ...
  mainLayout->addItem(
      new QSpacerItem(1, 1, QSizePolicy::MinimumExpanding, QSizePolicy::Fixed));

  connect(p1, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(textChanged(const QString&)));
  connect(p2, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(textChanged(const QString&)));
  connect(p3, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(textChanged(const QString&)));
  connect(p4, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(textChanged(const QString&)));
}
