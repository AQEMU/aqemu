    ...
    : QDialog(parent)
{
  ...
  ui.Edit_Template_Folder->setText(QDir::toNativeSeparators(
...
void Create_Template_Window::on_TB_Template_Folder_Browse_clicked()
{
  QString fileName = QFileDialog::getExistingDirectory(
      this, tr("Select Template Folder"),
...
void Create_Template_Window::on_Button_Create_clicked()
{
  ...
  if (ui.Edit_Template_Name->text().isEmpty()) {
    AQGraphic_Warning(tr("Warning!"), tr("Template Name is Empty!"));
    return;
  }
  ...
  if (!QFile::exists(ui.Edit_VM_File->text())) {
    AQGraphic_Warning(tr("Warning!"), tr("VM file doesn't exist!"));
    return;
  }

  if (!QFile::exists(ui.Edit_Template_Folder->text())) {
    AQGraphic_Warning(tr("Warning!"), tr("Cannot Locate Template Folder!"));
    return;
  }

  if (ui.Edit_Template_Folder->text().at(
          ui.Edit_Template_Folder->text().size() - 1) != '/') {
    ui.Edit_Template_Folder->setText(
        QDir::toNativeSeparators(ui.Edit_Template_Folder->text() + "/"));
  }
  ...
  if (!tmp_vm->Load_VM(ui.Edit_VM_File->text())) {
    AQGraphic_Warning(tr("Error!"), tr("Cannot Load VM!"));
    delete tmp_vm;
    return;
  }
  ...
  if (!tmp_vm->Create_Template(
          ui.Edit_Template_Folder->text() + new_template_name + ".aqvmt",
          ui.Edit_Template_Name->text(), tmp_options)) {
    AQGraphic_Warning(tr("Error!"), tr("Cannot Create Template!"));
    delete tmp_vm;
    return;
  }
    ...
    for (int ix = 0; ix < List_Templates.count(); ++ix) {
      ...
      if (tmp_info.completeBaseName() == new_template_name) {
        int mes_ret = QMessageBox::question(
            this, tr("Replace"),
            tr("Your template name is already used! Do you want replace "
               "previous template?"),
