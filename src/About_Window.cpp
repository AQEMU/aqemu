...
About_Window::About_Window(QWidget* parent) : QDialog(parent)
{
  ...
  ui.Edit_Thanks_To_Text->setHtml(tr(
      "<b>Developers:</b>\n"
      "<br>Andrey Rijov (aka RDron) - Original Author (Up until version "
      "0.8.2)\n"
      "<br>Tobias Gläßer - Maintainer/Developer (Version 0.9.0 and up)\n"
      "<br>Herman Semenov (GermanAizek@yandex.ru) - Maintainer/Developer "
      "AQEMU2 (Version 2.0.0 and up)\n"
      ...
      "<br>Michael Schmöller (aka schmoemi) - German Translation and "
      ...
      "<br><br><b>Thanks all www.nclug.ru group for testing and suggestions "
      "for improvement.</b>\n"

      "<br><br><b>AQEMU used code from QtEMU and Krdc thanks all "
      "developers:</b>\n"
  ...
  linksFilePath =
      QDir::toNativeSeparators(logFileDir.absolutePath() + "/links.html");

  Show_Links_File();

  ui.About_Text->setText(ui.About_Text->text().replace(
      "$VERSION_DATE_DO_NOT_TRANSLATE",
      QString(CURRENT_AQEMU_VERSION) + " (" +
          QString(CURRENT_AQEMU_RELEASE_DATE) + ")"));
...
void About_Window::Show_Links_File()
{
  ...
  else if (QFile::exists(QDir::toNativeSeparators(
               settings.value("AQEMU_Data_Folder", "").toString() +
               "/docs/links.html"))) {
    show_url = QDir::toNativeSeparators(
        settings.value("AQEMU_Data_Folder", "").toString() +
        "/docs/links.html");
  }
  else {
    AQGraphic_Warning(tr("Error!"), tr("Cannot Find AQEMU Links File!"));
    return;
  }
