...
void Create_HDD_Image_Window::on_CB_Format_currentIndexChanged(
    const QString& text)
{
...
void Create_HDD_Image_Window::on_Button_Create_clicked()
{
  ...
  if (ui.CH_Base_Image->isChecked()) {
    ...
    if (base_image_path.isRelative()) {
      base_image_path.setFile(
          QDir::cleanPath(ui.Edit_File_Name->text() + "/../" +
    ...
    if (!base_image_path.exists()) {
      ...
      Create_OK =
          Create_New_HDD_Image(false, ui.Edit_Base_Image_File_Name->text(),
                               ui.Edit_File_Name->text(),
                               ui.CB_Format->currentText(), hd_size, true);
    }
  }
  else {
    Create_OK =
        Create_New_HDD_Image(false, "", ui.Edit_File_Name->text(),
                             ui.CB_Format->currentText(), hd_size, true);
  }
...
void Create_HDD_Image_Window::on_Button_Format_Help_clicked()
{
  QMessageBox::information(
      this, tr("QEMU-IMG Supported formats"),
      tr("raw\nRaw disk image format. This format has the advantage of being "
         "simple and easily "
         "exportable to all other emulators. If your file system supports "
         "holes (for example in "
         "ext2 or ext3 on Linux or NTFS on Windows), then only the written "
         "sectors will reserve "
         "space. Use qemu-img info to know the real size used by the image or "
         "ls -ls on Unix/Linux."
         ...
         "only growable image "
         "format in QEMU. It is supported only for compatibility with previous "
         "versions. "
         "It does not work on win32."
         "\n\nvmdk\nVMware 3 and 4 compatible image format."
         "\n\ncloop\nLinux Compressed Loop image, useful only to reuse "
         "directly compressed CD-ROM "
