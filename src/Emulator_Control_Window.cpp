    ...
    : QMainWindow(parent)
{
  ...
  connect(ui.menuConnectNew, SIGNAL(aboutToShow()), this,
          SLOT(Get_Removable_Devices_List()));
...
void Emulator_Control_Window::VM_State_Changed(Virtual_Machine* vm,
                                               VM::VM_State state)
{
  switch (state) {
    case VM::VMS_Pause:
...
{
  connect(Cur_VM, SIGNAL(Ready_Removable_Devices_List()), this,
          SLOT(Create_Connect_Menu()));
...
{
  disconnect(Cur_VM, SIGNAL(Ready_Removable_Devices_List()), this,
             SLOT(Create_Connect_Menu()));
  ...
  for (qsizetype ix = 0; ix < devices.count() - 1; ++ix) {
    ...
    if (curDev[0].contains("-cd"))  // CD-ROM
    {
      ...
      // Create menu item
      devName = QString("CD-ROM %1 (On %2%3)")
                    .arg(nameStrings[4])
                    .arg(nameStrings[1].toUpper())
                    .arg(nameStrings[2]);
      QMenu* tmpMenu =
          ui.menuConnectNew->addMenu(QIcon(":/cdrom.png"), devName);
...
void Emulator_Control_Window::Create_Device_Menu()
{
  ...
  // Parse line
  QString internalDevName = QString(value).replace(QRegularExpression("\\:.*"),
                                                   "");  // Get device name
  ...
  if (fileIndex > 0) {
    ...
    if (fileIndex < value.size()) {
      for (int ix = fileIndex; ix < value.size(); ++ix) {
        ...
        {
          deviceSourcePath =
              value.mid(fileIndex, ix - fileIndex);  // Save file path
          break;
        }
      }
    }
  ...
  if (internalDevName.contains("-cd")) {
    ...
    if (!deviceSourcePath.isEmpty()) {
      ...
      QAction* conCdromAct =
          tmpMenu->addAction(tr("Current \"") + tmpInfo.fileName() + "\"", this,
                             SLOT(Connect_Device()));
    ...
    for (int ix = 0; ix < cdList.count(); ++ix) {
      if (cdList[ix] != deviceSourcePath) {
        QAction* conCdromAct = tmpMenu->addAction(
            tr("Connect \"") + cdList[ix] + "\"", this, SLOT(Connect_Device()));
        conCdromAct->setData(internalDevName + "\n" + cdList[ix]);
      }
    }
    ...
    for (int ix = 0; ix < cdList.count(); ++ix) {
      if (cdList[ix] != deviceSourcePath) {
        ...
        QAction* conCdromAct =
            tmpMenu->addAction(tr("Connect \"") + tmpInfo.fileName() + "\"",
    ...
    QAction* browseAct = tmpMenu->addAction(
        QIcon(":/open-file.png"), "Browse...", this, SLOT(Open_Device_File()));
    browseAct->setData(internalDevName);

    // Add 'Eject' menu item
    QAction* ejectAct = tmpMenu->addAction(QIcon(":/eject.png"), "Eject", this,
                                           SLOT(Eject_Device()));
    ...
    for (int ix = 0; ix < fddList.count(); ++ix) {
      if (fddList[ix] != deviceSourcePath) {
        QAction* conFddAct =
            tmpMenu->addAction(tr("Connect \"") + fddList[ix] + "\"", this,
                               SLOT(Connect_Device()));
        conFddAct->setData(internalDevName + "\n" + fddList[ix]);
      }
    }
...
void Emulator_Control_Window::Open_Device_File()
{
  ...
  QString fileName =
      QFileDialog::getOpenFileName(this, tr("Open Device or Image File"),
                                   Get_Last_Dir_Path(lastDir), fileFilter);
...
void Emulator_Control_Window::closeEvent(QCloseEvent* event)
{
  ...
  if (Use_VNC()) {
    int ret = QMessageBox::question(
        this, tr("Power Off This VM?"),
        tr("Selected VM is running now. Shutdown anyway?"),
        QMessageBox::Yes | QMessageBox::Save | QMessageBox::Close |
            QMessageBox::Cancel,
        QMessageBox::Cancel);
...
void Emulator_Control_Window::Set_Current_VM(Virtual_Machine* vm)
{
  ...
  for (int ix = 0; ix < Cur_VM->Get_USB_Ports().count(); ++ix) {
    QAction* new_act = new QAction(
        Cur_VM->Get_USB_Ports()[ix].Get_Product_Name(), ui.menuDisconnect);
...
void Emulator_Control_Window::Init()
{
  ...
  for (int ix = 0; ix < max; ++ix) {
    ...
    connect(Recent_Files_CD_Items[ix], SIGNAL(triggered()), this,
            SLOT(Open_Recent_CD_ROM_Image()));
  }
  ...
  for (qsizetype ix = 0; ix < max; ++ix) {
    ...
    connect(Recent_Files_FD0_Items[ix], SIGNAL(triggered()), this,
  ...
  for (qsizetype ix = 0; ix < max; ++ix) {
    ...
    connect(Recent_Files_FD1_Items[ix], SIGNAL(triggered()), this,
            SLOT(Open_Recent_Floppy1_Image()));
  }
...
// Use VNC Embedded Display
  ...
  if (!Use_VNC()) {
    ...
    connect(Machine_View, SIGNAL(Full_Size(int, int)), this,
            SLOT(Apply_Full_Size(int, int)));

    if (Cur_VM->Use_VNC())
      Machine_View->Set_VNC_URL("localhost",
                                Cur_VM->Get_VNC_Display_Number() + 5900);
    else
      Machine_View->Set_VNC_URL(
          "localhost",
          Cur_VM->Get_Embedded_Display_Port() +
...
void Emulator_Control_Window::on_actionSave_Screenshot_triggered()
{
  if (Settings.value("Use_Screenshots_Folder", "no").toString() == "yes") {
    if (!Settings.value("Screenshot_Folder_Path", "").toString().isEmpty()) {
      ...
      // save screenshot
      Cur_VM->Take_Screenshot(
          Settings.value("Screenshot_Folder_Path", "").toString() + unic_name +
          "." + Settings.value("Screenshot_Save_Format", "PNG").toString());
    }
  }
  else {
    AQGraphic_Warning("Warning!",
                      "Shared Screenshots Folder Not Enabled!\nPlease Enter "
                      "Name for New Screenshot.");
...
void Emulator_Control_Window::on_actionShutdown_triggered()
{
  if (QMessageBox::question(this, tr("Are you sure?"),
                            tr("Are you sure you want to shutdown VM \"%1\"?")
...
{
  if (QMessageBox::question(this, tr("Are you sure?"),
                            tr("Are you sure you want to poweroff VM \"%1\"?")
                                .arg(Cur_VM->Get_Machine_Name()),
                            QMessageBox::Yes | QMessageBox::No,
                            QMessageBox::Yes) == QMessageBox::No) {
    return;
...
{
  if (QMessageBox::question(this, tr("Are you sure?"),
                            tr("Are you sure you want to reset VM \"%1\"?")
                                .arg(Cur_VM->Get_Machine_Name()),
                            QMessageBox::Yes | QMessageBox::No,
                            QMessageBox::Yes) == QMessageBox::No) {
    return;
...
void Emulator_Control_Window::on_actionFD0_Other_triggered()
{
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Open Device or Image File"),
  ...
  if (usb_item) {
    ...
    for (int ix = 0; ix < ac_list.count(); ++ix) {
      if (usb_item->data().toString() == ac_list[ix]->data().toString()) {
        ...
        QString
            bus_addr;  // FIXME = Cur_VM->Get_USB_Bus_Address( usb_item->data().toString() );
...
void Emulator_Control_Window::on_actionUSB_Update_Device_List_triggered()
{
  ...
  for (int ix = 0; ix < tmp_list.count(); ++ix) {
    QAction* new_act =
        new QAction(tmp_list[ix].Get_Product_Name(), ui.menuUSB_Connect);
...
void Emulator_Control_Window::on_actionBy_Bus_Address_triggered()
{
  ...
  QString bus_addr = QInputDialog::getText(
      this, tr("Disconnect USB"), tr("Enter You Bus.Address VM USB Value"),
      QLineEdit::Normal, "", &ok);
...
void Emulator_Control_Window::on_actionUSB_Disconnect_All_Devices_triggered()
{
  ...
  for (int dx = 0; dx < ac_list.count(); ++dx) {
    if (ac_list[dx]->data().isNull() == false &&
      ...
      QString
          bus_addr;  // FIXME = Cur_VM->Get_USB_Bus_Address( ac_list[dx]->data().toString() );
...
void Emulator_Control_Window::on_actionOther_Keys_triggered()
{
  ...
  QString keys = QInputDialog::getText(
      this, tr("Send Keys"),
...
void Emulator_Control_Window::on_actionFullscreen_Mode_triggered()
{
  ...
  if (ui.actionFullscreen_Mode->isChecked()) {
    if (Settings.value("Show_Fullscreen_Warning", "yes").toString() == "yes") {
      int ret = QMessageBox::question(
          this, tr("Fullscreen mode"),
          tr("To return from fullscreen to windowed mode press "
...
bool Emulator_Control_Window::Use_VNC()
{
  if (Settings.value("Use_VNC_Display", "no").toString() == "yes" &&
      Cur_VM->Get_VNC_Socket_Mode() == false &&
      Cur_VM->Get_Video_Card() !=
          "none")  //TODO: Should also work seemlessly if VNC is set to use a socket
  {
    return true;
  }
  else {
...
void Emulator_Control_Window::on_actionAbout_Emulator_Control_triggered()
{
  QMessageBox::information(this, tr("About Emulator Control..."),
                           tr("Emulator Control is a Tool for Managing a "
...
bool Emulator_Control_Window::FD0_Available()
{
  ...
  if (Cur_VM->Get_FD0().Get_Enabled() == false) {
    QMessageBox::information(this, tr("Warning!"),
                             tr("In This VM Floppy 1 Not Found!"));
    return false;
  }
  else {
    return true;
...
bool Emulator_Control_Window::FD1_Available()
{
  ...
  if (Cur_VM->Get_FD1().Get_Enabled() == false) {
    QMessageBox::information(this, tr("Warning!"),
                             tr("In This VM Floppy 2 Not Found!"));
    return false;
  }
  else {
    return true;
...
bool Emulator_Control_Window::CD_ROM_Available()
{
  ...
  if (Cur_VM->Get_HDC().Get_Enabled() == true ||
    ...
    QMessageBox::information(this, tr("Warning!"),
                             tr("In This VM CD-ROM Not Found!"));
    return false;
  }
  else {
    return true;
...
void Emulator_Control_Window::Update_Recent_CD_ROM_Images_List()
{
  ...
  while (max > Recent_Files_CD_Items.count()) {
    ...
    connect(Recent_Files_CD_Items[Recent_Files_CD_Items.count() - 1],
            SIGNAL(triggered()), this, SLOT(Open_Recent_CD_ROM_Image()));
  }

  for (int ix = 0; ix < max; ++ix) {
    if (ix < Get_CD_Recent_Images_List().count() &&
        Get_CD_Recent_Images_List()[ix].isEmpty() == false) {
...
void Emulator_Control_Window::Update_Recent_Floppy_Images_List()
{
  ...
  while (max > Recent_Files_FD0_Items.count()) {
    ...
    Recent_Files_FD0_Items[Recent_Files_FD0_Items.count() - 1]->setVisible(
        false);

    connect(Recent_Files_FD0_Items[Recent_Files_FD0_Items.count() - 1],
            SIGNAL(triggered()), this, SLOT(Open_Recent_Floppy0_Image()));
  }
  ...
  while (max > Recent_Files_FD1_Items.count()) {
    ...
    Recent_Files_FD1_Items[Recent_Files_FD1_Items.count() - 1]->setVisible(
        false);

    connect(Recent_Files_FD1_Items[Recent_Files_FD1_Items.count() - 1],
            SIGNAL(triggered()), this, SLOT(Open_Recent_Floppy1_Image()));
  }
                                         ...
                                         const QString& path)
{
  ...
  if (dev_name == "fda")
    ...
    if (dev_name.contains("-cd"))  // CD-ROM?
    {
      ...
      {
        Cur_VM->Set_CD_ROM(VM_Storage_Device(
            true, path, Cur_VM->Get_CD_ROM().Get_Native_Mode(),
            Cur_VM->Get_CD_ROM().Get_Native_Device()));
      }
      else {
        // Find CD-ROM in other storage devices
        QList<VM_Native_Storage_Device> devList =
            Cur_VM->Get_Storage_Devices_List();
    ...
    else if (dev_name.contains("floppy"))  // Floppy?
    {
      if (dev_name == "floppy0") {
        Cur_VM->Set_FD0(
            VM_Storage_Device(true, path, Cur_VM->Get_FD0().Get_Native_Mode(),
                              Cur_VM->Get_FD0().Get_Native_Device()));
      }
      else if (dev_name == "floppy1") {
        Cur_VM->Set_FD1(
            VM_Storage_Device(true, path, Cur_VM->Get_FD1().Get_Native_Mode(),
