...
Old_Network_Widget::Old_Network_Widget(QWidget* parent) : QWidget(parent)
{
  ...
  QRegularExpression rx(
      "^([1-9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-5])(\\.([0-9]|[1-9]["
      "\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-5])){3}$");
...
void Old_Network_Widget::Set_Network_Card_Models(
    const QList<Device_Map>& models)
{
  if (ui.CB_Network_Card_Model->count() > 0)
...
{
  connect(ui.CB_Connection_Mode, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.CB_Network_Card_Model, SIGNAL(currentIndexChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.Edit_Hostname, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.Edit_IP_Address, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.Edit_MAC_Address, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.SB_File_Descriptor, SIGNAL(valueChanged(int)), this,
          SIGNAL(Changed()));

  connect(ui.SB_Port, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  connect(ui.SB_VLAN, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  connect(ui.CH_TUN_TAP_Script, SIGNAL(clicked()), this, SIGNAL(Changed()));

  connect(ui.Edit_TUN_TAP_Script, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));

  connect(ui.Edit_Interface_Name, SIGNAL(textChanged(const QString&)), this,
          SIGNAL(Changed()));
}

void Old_Network_Widget::Disconnect_Slots()
{
  disconnect(ui.CB_Connection_Mode, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.CB_Network_Card_Model, SIGNAL(currentIndexChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.Edit_Hostname, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.Edit_IP_Address, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.Edit_MAC_Address, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.SB_File_Descriptor, SIGNAL(valueChanged(int)), this,
             SIGNAL(Changed()));

  disconnect(ui.SB_Port, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  disconnect(ui.SB_VLAN, SIGNAL(valueChanged(int)), this, SIGNAL(Changed()));

  disconnect(ui.CH_TUN_TAP_Script, SIGNAL(clicked()), this, SIGNAL(Changed()));

  disconnect(ui.Edit_TUN_TAP_Script, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));

  disconnect(ui.Edit_Interface_Name, SIGNAL(textChanged(const QString&)), this,
             SIGNAL(Changed()));
}

void Old_Network_Widget::on_Network_Cards_List_currentItemChanged(
    QListWidgetItem* current, QListWidgetItem* previous)
{
  if (Network_Cards.count() < 1 || Check_Network_Card == false)
    return;
...
void Old_Network_Widget::on_Button_Add_Net_Card_clicked()
{
  if (ui.Network_Cards_List->count() >= 8)
    ...
    new QListWidgetItem(
        tr("Card ") + QString::number(ui.Network_Cards_List->count() + 1),
        ui.Network_Cards_List);
...
void Old_Network_Widget::on_Button_Delete_Net_Card_clicked()
{
  ...
  else if (ui.Network_Cards_List->currentRow() <
               ui.Network_Cards_List->count() &&
...
{
  if (index < 0 || Network_Cards.count() <= 0 ||
...
void Old_Network_Widget::on_CB_Connection_Mode_currentIndexChanged(int index)
{
  ...
  switch (index) {
    case 0:  // user mode
      if (set_net_mode)
        Network_Cards[ui.Network_Cards_List->currentRow()].Set_Net_Mode(
            VM::Net_Mode_Usermode);
...
{
  Network_Cards[ui.Network_Cards_List->currentRow()].Set_Hostname(
      ui.Edit_Hostname->text());
}

void Old_Network_Widget::on_Edit_IP_Address_textChanged()
{
  Network_Cards[ui.Network_Cards_List->currentRow()].Set_IP_Address(
      ui.Edit_IP_Address->text());
}

void Old_Network_Widget::on_Edit_MAC_Address_textChanged()
{
  Network_Cards[ui.Network_Cards_List->currentRow()].Set_MAC_Address(
      ui.Edit_MAC_Address->text());
}
...
{
  if (state == Qt::Checked)
    Network_Cards[ui.Network_Cards_List->currentRow()].Set_Use_TUN_TAP_Script(
        true);
...
{
  Network_Cards[ui.Network_Cards_List->currentRow()].Set_TUN_TAP_Script(
      ui.Edit_TUN_TAP_Script->text());
}

void Old_Network_Widget::on_Edit_Interface_Name_textChanged()
{
  Network_Cards[ui.Network_Cards_List->currentRow()].Set_Interface_Name(
      ui.Edit_Interface_Name->text());
}
...
{
  ui.Edit_MAC_Address->setText(
      Network_Cards[ui.Network_Cards_List->currentRow()].Generate_MAC());
}

void Old_Network_Widget::on_TB_Browse_TUN_Script_clicked()
{
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Select script"),
...
bool Old_Network_Widget::Net_Card_is_Valid(int index)
{
  ...
  if (Network_Cards[index].Get_Net_Mode() == VM::Net_Mode_Tcplisten ||
    ...
    if (!rx_ip.match(Network_Cards[index].Get_IP_Address()).hasMatch()) {
      if (!(Network_Cards[index].Get_Net_Mode() == VM::Net_Mode_Tcplisten &&
        ...
        AQGraphic_Warning(tr("Error!"), tr("IP Address in Card ") +
  ...
  if (Network_Cards[index].Get_Net_Mode() == VM::Net_Mode_Usermode ||
    ...
    if (!Network_Cards[index].Get_MAC_Address().isEmpty() &&
      ...
      AQGraphic_Warning(tr("Error!"), tr("MAC Address in Card ") +
                                          QString::number(index + 1) +
                                          tr(" Invalid!"));
      return false;
    }
  ...
  if (Network_Cards[index].Get_Net_Mode() == VM::Net_Mode_Tuntap) {
    if (Network_Cards[index].Get_Use_TUN_TAP_Script() == true &&
      ...
      int retVal = QMessageBox::critical(
          this, tr("Error!"),
          tr("In Card ") + QString::number(index + 1) +
              tr(" TUN/TAP Script File \"%1\" Not Exists!\nContinue Without It "
                 "File?")
                  .arg(Network_Cards[index].Get_TUN_TAP_Script()),
