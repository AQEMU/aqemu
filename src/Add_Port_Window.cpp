...
Add_Port_Window::Add_Port_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);
...
VM_Port Add_Port_Window::Get_Port() const
{
  ...
  if (ui.RB_Default->isChecked())
    port.Set_Port_Redirection(VM::PR_Default);
  else if (ui.RB_vc->isChecked())
    port.Set_Port_Redirection(VM::PR_vc);
  else if (ui.RB_pty->isChecked())
    port.Set_Port_Redirection(VM::PR_pty);
  else if (ui.RB_none->isChecked())
    port.Set_Port_Redirection(VM::PR_none);
  else if (ui.RB_null->isChecked())
    port.Set_Port_Redirection(VM::PR_null);
  else if (ui.RB_stdio->isChecked())
    port.Set_Port_Redirection(VM::PR_stdio);
  else if (ui.RB_Dev->isChecked())
    port.Set_Port_Redirection(VM::PR_dev);
  else if (ui.RB_File->isChecked())
    port.Set_Port_Redirection(VM::PR_file);
  else if (ui.RB_Pipe->isChecked())
    port.Set_Port_Redirection(VM::PR_pipe);
  else if (ui.RB_UDP->isChecked())
    port.Set_Port_Redirection(VM::PR_udp);
  else if (ui.RB_TCP->isChecked())
    port.Set_Port_Redirection(VM::PR_tcp);
  else if (ui.RB_Telnet->isChecked())
    port.Set_Port_Redirection(VM::PR_telnet);
  else if (ui.RB_Unix->isChecked())
    port.Set_Port_Redirection(VM::PR_unix);
  else if (ui.RB_COM->isChecked())
    port.Set_Port_Redirection(VM::PR_com);
  else if (ui.RB_msmouse->isChecked())
    port.Set_Port_Redirection(VM::PR_msmouse);
  else if (ui.RB_mon->isChecked())
    port.Set_Port_Redirection(VM::PR_mon);
  else if (ui.RB_braille->isChecked())
    port.Set_Port_Redirection(VM::PR_braille);
  else {
...
void Add_Port_Window::on_TB_Browse_clicked()
{
  QString po = QFileDialog::getOpenFileName(
      this, tr("Open File"), Get_Last_Dir_Path(ui.Edit_Args->text()),
      tr("All Files (*)"));
...
void Add_Port_Window::Show_Arg_Help()
{
  ...
  argsHelp << tr("");
  argsHelp << tr(
      "Optionally width and height can be given in pixel with\n800x600\nIt is "
      "also possible to specify width or height in characters:\n80Cx24C");
  argsHelp << tr(
  ...
  argsHelp << tr(
      "filename - Write output to filename. No character can be read.");
  argsHelp << tr("filename - name pipe filename");
  argsHelp << tr(
      "[remote_host]:remote_port[@[src_ip]:src_port]\nThis implements UDP Net "
      "Console. When remote_host or src_ip are not specified they default to "
      "0.0.0.0. When not using a specified src_port a random port is "
      "automatically chosen.");
  argsHelp << tr(
      "[host]:port[,server][,nowait][,nodelay]\nExample to send tcp console to "
      "192.168.0.2 port 4444\n92.168.0.2:4444\nExample to listen and wait on "
      "port 4444 for connection\n:4444,server\nExample to not wait and listen "
      "on ip 192.168.0.100 port 4444\n192.168.0.100:4444,server,nowait");
  argsHelp << tr(
      "host:port[,server][,nowait][,nodelay]\nThe telnet protocol is used "
      "instead of raw tcp sockets.");
  argsHelp << tr(
      "path[,server][,nowait]\nA unix domain socket is used instead of a tcp "
      "socket. The option works the same as if you had specified -serial tcp "
  ...
  argsHelp << tr(
      ...
      "multiplexed onto another serial port.");
  argsHelp << tr(
      "Braille device.\nThis will use BrlAPI to display the braille output on "
      "a real or fake device.");

  if (ui.RB_Default->isChecked())
    ui.Label_Help->setText(argsHelp[0]);
  else if (ui.RB_vc->isChecked())
    ui.Label_Help->setText(argsHelp[1]);
  else if (ui.RB_pty->isChecked())
    ui.Label_Help->setText(argsHelp[2]);
  else if (ui.RB_none->isChecked())
    ui.Label_Help->setText(argsHelp[3]);
  else if (ui.RB_null->isChecked())
    ui.Label_Help->setText(argsHelp[4]);
  else if (ui.RB_stdio->isChecked())
    ui.Label_Help->setText(argsHelp[5]);
  else if (ui.RB_Dev->isChecked())
    ui.Label_Help->setText(argsHelp[6]);
  else if (ui.RB_COM->isChecked())
    ui.Label_Help->setText(argsHelp[7]);
  else if (ui.RB_File->isChecked())
    ui.Label_Help->setText(argsHelp[8]);
  else if (ui.RB_Pipe->isChecked())
    ui.Label_Help->setText(argsHelp[9]);
  else if (ui.RB_UDP->isChecked())
    ui.Label_Help->setText(argsHelp[10]);
  else if (ui.RB_TCP->isChecked())
    ui.Label_Help->setText(argsHelp[11]);
  else if (ui.RB_Telnet->isChecked())
    ui.Label_Help->setText(argsHelp[12]);
  else if (ui.RB_Unix->isChecked())
    ui.Label_Help->setText(argsHelp[13]);
  else if (ui.RB_msmouse->isChecked())
    ui.Label_Help->setText(argsHelp[14]);
  else if (ui.RB_mon->isChecked())
    ui.Label_Help->setText(argsHelp[15]);
  else if (ui.RB_braille->isChecked())
    ui.Label_Help->setText(argsHelp[16]);
  else {
