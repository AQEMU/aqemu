...
Add_USB_Port_Window::Add_USB_Port_Window(QWidget* parent) : QDialog(parent)
{
  ui.setupUi(this);
...
{
  if (ui.RB_QEMU_USB->isChecked())
    Current_Item.Set_Use_Host_Device(false);
  else
    Current_Item.Set_Use_Host_Device(true);

  bool usb_k, usb_m, usb_t, usb_wt, usb_b;
  usb_k = usb_m = usb_t = usb_wt = usb_b = false;

  if (ui.RB_QEMU_Keyboard->isChecked())
    usb_k = true;
  else if (ui.RB_QEMU_Mouse->isChecked())
    usb_m = true;
  else if (ui.RB_QEMU_Tablet->isChecked())
    usb_t = true;
  else if (ui.RB_QEMU_WacomTablet->isChecked())
    usb_wt = true;
  else if (ui.RB_QEMU_Braille->isChecked())
    usb_b = true;
...
void Add_USB_Port_Window::Set_Port(const VM_USB& port)
{
  ...
  if (port.Get_Use_Host_Device() == false)
    on_RB_QEMU_USB_clicked(true);
  else
    on_RB_Host_USB_clicked(true);
  ...
  if (usb_k)
    ui.RB_QEMU_Keyboard->setChecked(true);
  else if (usb_m)
    ui.RB_QEMU_Mouse->setChecked(true);
  else if (usb_t)
    ui.RB_QEMU_Tablet->setChecked(true);
  else if (usb_wt)
    ui.RB_QEMU_WacomTablet->setChecked(true);
  else if (usb_b)
